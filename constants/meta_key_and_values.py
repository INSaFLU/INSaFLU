...
class MetaKeyAndValue(object):
    ...
    ## Value: META_VALUE_Error|META_VALUE_Success; Description: result.Result
    META_KEY_NanoStat_NanoFilt = (
        "NanoStatNanoFilt"  ## Used to run NanoStat and NanoFilt errors
    )
    META_KEY_NanoStat_NanoFilt_Software = (
        "NanoStatNanoFiltSoftware"  ## Used to run NanoStat and NanoFilt
    )
    ...
    META_KEY_bam_stats = (
        "Bam statistics"  ## Has the bam statistics, reads available, mapped
    )
    ...
    META_KEY_Count_Samples_Var_Graph = (
        "Count_samples_var_graph"  ## has the number of samples in var graph
    )
    ...
    META_KEY_ALERT_REMOVE_ORIGINAL_FASTQ_FILES = (
        "Remove Original Fastq Files"  ### remove original fastq files
    )
    META_KEY_ALERT_NO_READS_AFTER_FILTERING = "No reads after filtering."

    #### KEYS to remove if we run Snippy And FreeBayes Again
    VECT_TO_REMOVE_RUN_PROJECT_SAMPLE = [
        META_KEY_ALERT_COVERAGE,
    ...
    VECT_MESSAGE_ALERT_COVERAGE = [
        META_KEY_ALERT_COVERAGE_value_defined_by_user,
    ...
    NAME_project_sample = (
        "ProjectSample"  ### if the software was used in project sample
    )

    ### order out
    VECT_TECHNOLOGIES_OUT_REPORT = [
    ...
    DICT_SOFTWARE_SHOW_IN_RESULTS = {
        ConstantsSettings.TECHNOLOGY_illumina: {
            NAME_sample: [
                META_KEY_Fastq_Trimmomatic_Software,
                META_KEY_Identify_Sample_Software,
            ],
            NAME_project_sample: [
                META_KEY_Snippy_Freebayes,
            ],
        },
        ConstantsSettings.TECHNOLOGY_minion: {
            NAME_sample: [
    ...
    META_KEY_Masking_consensus_by_minfrac_VCF_medaka = (
        "Masking consensus by Minfrac VCF medaka"
    )
    ...
    META_KEY_Queue_TaskID_ProjectSample = (
        "TaskID_ProjectSample"  ### has the metaKey plus project_sample_id
    )
    META_KEY_Queue_TaskID_Project = (
        "TaskID_Project"  ### has the metaKey plus project_sample_id
    )
    META_KEY_Elements_Reference = "Elements"  ### has the elements to a specific project, separated by comma and sorted
    META_KEY_Elements_And_CDS_Reference = (
        "Elements And CDS"  ### has the elements and the CDS
    )
    ...
    META_KEY_Dataset_max_name_length = (
        "Dataset_max_name_length"  ### has the max sample length for a specific dataset
    )

    ### MetaKey for the alerts
    META_KEY_Alert_First_level = (
        "AlertFirstLevel"  ### has the alert description for highest level alert
    )
    META_KEY_Alert_Second_level = (
        "AlertSecondLevel"  ### has the alert description for lowest level alert
    )
    ...
    def get_meta_key_queue_by_project_sample_id(self, project_sample_id):
        ...
        return self.get_meta_key(
            self.META_KEY_Queue_TaskID_ProjectSample, project_sample_id
        )
    ...
    def get_keys_show_alerts_in_sample_projects_details_view(self):
        ...
        """
        return [
            MetaKeyAndValue.META_KEY_ALERT_MIXED_INFECTION_RATIO_TEST,
            MetaKeyAndValue.META_KEY_ALERT_MIXED_INFECTION_SUM_TEST,
    ...
    def get_keys_show_alerts_in_sample_details_view(self):
        ...
        return [
            MetaKeyAndValue.META_KEY_ALERT_MIXED_INFECTION_TYPE_SUBTYPE,
            MetaKeyAndValue.META_KEY_ALERT_NOT_ASSIGNED_TYPE_SUBTYPE,
            MetaKeyAndValue.META_KEY_ALERT_DOWNSIZE_OF_FASTQ_FILES,
            MetaKeyAndValue.META_KEY_ALERT_REMOVE_ORIGINAL_FASTQ_FILES,
