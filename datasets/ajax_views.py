...
def remove_dataset(request):
    ...
    if request.is_ajax():
        ...
        if dataset_id_a in request.GET:
            ...
            if dataset.owner.pk != request.user.pk:
                return JsonResponse(data)
...
def add_dataset_name(request):
    ...
    if request.is_ajax():
        ...
        if dataset_name in request.GET:
            ...
            if len(dataset_name_str) == 0:
                return JsonResponse(data)

            try:
                dataset = Dataset.objects.get(
                    name=dataset_name_str, owner__pk=request.user.pk, is_deleted=False
                )

                ### name exist
                data["message"] = "This name '{}' already exist.".format(
                    dataset_name_str
                )
                return JsonResponse(data)
            except Dataset.DoesNotExist:
            ...
            try:
                ...
                vect_parameters = default_parameters.get_nextstrain_default(
                ...
                default_parameters.persist_parameters(
                    vect_parameters, type_of_use=Software.TYPE_OF_USE_dataset
                )

            except Exception as e:
                message = "The Dataset '{}' was created, but with problems: {}".format(
                    dataset_name_str, str(e)
                )
            ...
            data = {
                ...
                "id": dataset.pk,
                "date_created": dataset.creation_date.strftime(
                    settings.DATETIME_FORMAT_FOR_TABLE
                ),
                "message": message,
            }
        return JsonResponse(data)
    ...
    if request.is_ajax():
        ...
        if dataset_name in request.GET:
            ...
            try:
                ...
                data = {
                    "is_taken": True,
                    "error_message": "This name '{}' already exists".format(
                        dataset_name_str
                    ),
                }
            except Dataset.DoesNotExist:
...
def add_consensus_name(request):
    ...
    if request.is_ajax():
        ...
        if consensus_name in request.POST:
            ...
            try:
                dataset = Dataset.objects.get(
                    name=consensus_name_str, owner__pk=request.user.pk, is_deleted=False
                )
...
def remove_consensus(request):
    ...
    if request.is_ajax():
        ...
        if consensus_id_a in request.GET:
            ...
            ## different owner or belong to a project not deleted
            if consensus.owner.pk != request.user.pk:
                return JsonResponse(data)
    ...
    if request.is_ajax():
        ...
        if consensus_id_a in request.GET:
            ...
            if not dataset_consensus.project_sample is None:
                ...
                data["message"] = (
                    "The consensus sequence {} from project {} was removed".format(
...
def validate_consensus_name(request):
    ...
    if request.is_ajax():
        ...
        data = {
            "is_taken": Consensus.objects.filter(
                name__iexact=consensus_name, is_deleted=False, owner=request.user
            ).exists()
        }
        if data["is_taken"]:
            data["error_message"] = "Exists a consensus with this name."
        return JsonResponse(data)
    ...
    if request.is_ajax():
        ...
        if key_with_dataset_id in request.GET:
            ...
            try:
                ...
                dataset = Dataset.objects.get(id=dataset_id)
                file_name_fasta = dataset.get_global_file_by_dataset(
                    TypePath.MEDIA_ROOT, Dataset.DATASET_FILE_NAME_MAFFT
                )

                if os.path.exists(file_name_fasta):
                    file_name_fasta = dataset.get_global_file_by_dataset(
                        TypePath.MEDIA_URL, Dataset.DATASET_FILE_NAME_MAFFT
                    )
                    ...
                    url_file_name_fasta = '<a href="{}" download="{}"> {}</a>'.format(
                        file_name_fasta,
                        os.path.basename(file_name_fasta),
                        os.path.basename(file_name_fasta),
                    )
                else:
                ...
                file_name_nex = dataset.get_global_file_by_dataset(
                    TypePath.MEDIA_ROOT, Dataset.DATASET_FILE_NAME_nex
                )
                if os.path.exists(file_name_nex):
                    file_name_nex = dataset.get_global_file_by_dataset(
                        TypePath.MEDIA_URL, Dataset.DATASET_FILE_NAME_nex
                    )
                    url_file_name_nex = '<a href="{}" download="{}"> {}</a>'.format(
                        file_name_nex,
                        os.path.basename(file_name_fasta),
                        os.path.basename(file_name_nex),
                    )
                else:
                ...
                data["max_length_label"] = manage_database.get_max_length_label(
                    dataset, request.user, b_calculate_again
                )
            except Dataset.DoesNotExist:
                pass
        return JsonResponse(data)
    ...
    if request.is_ajax():
        ...
        if key_with_dataset_id in request.GET:
            ...
            try:
                ...
                dataset = Dataset.objects.get(id=dataset_id)
                file_name_root_json = dataset.get_global_file_by_dataset(
                    TypePath.MEDIA_ROOT, Dataset.DATASET_FILE_NAME_RESULT_json
                )
                file_name_url_json = dataset.get_global_file_by_dataset(
                    TypePath.MEDIA_URL, Dataset.DATASET_FILE_NAME_RESULT_json
                )
                ...
                file_name_root_nwk = dataset.get_global_file_by_dataset(
                    TypePath.MEDIA_ROOT, Dataset.DATASET_FILE_NAME_FASTTREE_tree
                )
                file_name_nwk = dataset.get_global_file_by_dataset(
                    TypePath.MEDIA_URL, Dataset.DATASET_FILE_NAME_FASTTREE
                )
                file_name_tree = dataset.get_global_file_by_dataset(
                    TypePath.MEDIA_URL, Dataset.DATASET_FILE_NAME_FASTTREE_tree
                )
                    ...
                    if (
                        ...
                        with open(
                            ...
                            if len(dt_result) == len(all_data):
                                ...
                                logger.error(
                                    "DatasetID: {}  different number of lines processing Sample {} -> JSON {}".format(
