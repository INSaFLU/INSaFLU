...
from datasets.models import Consensus, Dataset, UploadFiles
from managing_files.models import Project, Reference
...
class ConsensusForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        field_text = [
            ...
            (
                "consensus_fasta",
                "Consensus (Fasta/Multi-Fasta)",
                "Consensus file in fasta format.<br>"
                + "Max total sequence length: {}<br>".format(
                    filesizeformat(
                        settings.MAX_LENGTH_SEQUENCE_TOTAL_FROM_CONSENSUS_FASTA
                    )
                )
                + "Max FASTA file size: {}".format(
                    filesizeformat(settings.MAX_CONSENSUS_FASTA_FILE)
                ),
                True,
            ),
        ]
    ...
    def clean(self):
        ...
        if "consensus_fasta" not in cleaned_data:
            self.add_error(
                "consensus_fasta", "Error: Must have a Fasta/Multi-Fasta file."
            )
            return cleaned_data
        ...
        try:
            ...
            if not b_pass:
                ...
                for message in vect_error:
                    self.add_error("consensus_fasta", message)
            else:
                ## if empty load all
...
class DatastesUploadDescriptionMetadataForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            HTML("<p> </p>"),
            HTML(
                '<div class="alert alert-dark"> <span><i class="fa fa-download"></i></span> '
                + dataset_name.get_global_file_by_dataset_web(
                    Dataset.DATASET_FILE_NAME_RESULT_NEXTSTRAIN_TSV
                )
            ...
            ButtonHolder(
                ...
                Button(
                    ...
                    css_class="btn-secondary",
                    onclick='window.location.href="{}"'.format(
                        reverse("dataset-update-metadata", args=[self.pk])
                    ),
                ),
            ),
        )
