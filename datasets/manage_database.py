...
class ManageDatabase(object):
    ...
    def update_dataset_metakey(self, dataset, owner, meta_key_name, value, description):
        ...
        metaKeyDataset_list = MetaKeyDataset.objects.filter(
            dataset=dataset, meta_tag=metaKey, owner=owner
        )
        if metaKeyDataset_list.count() == 0:
            return self.set_dataset_metakey(
                dataset, owner, meta_key_name, value, description
            )
        else:
    ...
    def get_dataset_metakey(self, dataset, meta_key_name, value):
        ...
        try:
            if value == None:
                return MetaKeyDataset.objects.filter(
                    dataset__id=dataset.id, meta_tag__name=meta_key_name
                ).order_by("-creation_date")
            return MetaKeyDataset.objects.get(
                dataset__id=dataset.id, meta_tag__name=meta_key_name, value=value
            )
    ...
    def get_dataset_metakey_last(self, dataset, meta_key_name, value):
        ...
        try:
            if value == None:
                query_set = MetaKeyDataset.objects.filter(
                    dataset__id=dataset.id, meta_tag__name=meta_key_name
                ).order_by("-creation_date")
            else:
                query_set = MetaKeyDataset.objects.filter(
    ...
    def is_dataset_processing_step(self, dataset, tag_key):
        ...
        meta_dataset_queue = self.get_dataset_metakey_last(
            dataset, tag_key, MetaKeyAndValue.META_VALUE_Queue
        )
        if meta_dataset_queue is None:
            return True

        ### try to find queue_success
        meta_dataset_queue_success = self.get_dataset_metakey_last(
            dataset, tag_key, MetaKeyAndValue.META_VALUE_Success
        )
        if meta_dataset_queue_success is None:
            return True
        if (
            meta_dataset_queue_success.creation_date > meta_dataset_queue.creation_date
            and meta_dataset_queue_success.description == meta_dataset_queue.description
        ):
            return False
        return True
    ...
    def get_max_length_label(self, dataset, user, b_calculate_again):
        ...
        if not b_calculate_again:
            meta_data = self.get_dataset_metakey_last(
                dataset,
                MetaKeyAndValue.META_KEY_Dataset_max_name_length,
        ...
        for dataset_consensus in dataset.dataset_consensus.all():
            ...
            if dataset_consensus.is_error:
                continue
            if len(dataset_consensus.get_name()) > n_max_value:
                n_max_value = len(dataset_consensus.get_name())

        meta_data = self.update_dataset_metakey(
            dataset,
            user,
            MetaKeyAndValue.META_KEY_Dataset_max_name_length,
            MetaKeyAndValue.META_VALUE_Success,
