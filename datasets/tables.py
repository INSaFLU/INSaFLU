...
from managing_files.models import Project, Reference, UploadFiles
...
class DatasetTable(tables.Table):
    ...
    def render_name(self, record):
        ...
        count = (
            record.number_of_sequences_from_projects
            + record.number_of_sequences_from_consensus
            + record.number_of_sequences_from_references
        )
        if count > 0:
            project_sample = (
                "<a href="
                + reverse("show-dataset-consensus", args=[record.pk])
                + ' data-toggle="tooltip" title="Show sequences">'
        ...
        if user.username == record.owner.username:
            return mark_safe(
                '<a href="#id_remove_modal" id="id_remove_dataset_modal" data-toggle="modal" data-toggle="tooltip" title="Delete"'
                + ' ref_name="'
                + record.name
                + '" pk="'
                + str(record.pk)
    ...
    def render_sequences(self, record):
        ...
        add_sequence = (
            ...
            + reverse("add-references-dataset", args=[record.pk])
            + "> Add References</a>"
            + '<a rel="nofollow" class="dropdown-item" href='
            + reverse("add-projects-dataset", args=[record.pk])
            + " > Add Consensus from Projects</a>"
            + '<a rel="nofollow" class="dropdown-item" href='
            + reverse("add-consensus-dataset", args=[record.pk])
            + " > Add your own Consensus</a>"
            + "</div> </div>"
        )
        ...
        if count > 0:
            ...
            if record.is_processed:
                if user.username != Constants.USER_ANONYMOUS:
                    ...
                    sz_project_sample += (
                        '<a href="#id_rebuild_modal" id="id_rebuild_dataset_modal" data-toggle="modal" title="Rebuild Dataset Results"'
                        + ' ref_name="'
                        + record.name
                        + '" pk="'
                        + str(record.pk)
                        + '"><span ><i class="fa fa-flask" style="color:#55aa55;"></i></span> </a>'
                    )
            else:
                sz_project_sample += (
                    '<a href="#id_rebuild_modal" id="id_rebuild_dataset_modal" data-toggle="modal" title="Waiting for Dataset Results (click to rebuild at your own risk)"'
                    + ' ref_name="'
                    + record.name
                    + '" pk="'
                    + str(record.pk)
                    + '"><span ><i class="fa fa-spinner fa-spin" style="color:#ff0000;"></i></span> </a>'
                )
...
class ConsensusTable(tables.Table):
    ...
    consensus_fasta_name = tables.Column(verbose_name="Fasta file")
    owner = tables.Column("Owner", orderable=True, empty_values=())
    constants = Constants()
    ...
    def render_name(self, record):
        ...
        if (
            ...
            and DatasetConsensus.objects.filter(
                consensus__pk=record.pk, dataset__is_deleted=False, is_deleted=False
            ).count()
            == 0
        ):
            return mark_safe(
                '<a href="#id_remove_modal" id="id_remove_consensus_modal" data-toggle="modal" data-toggle="tooltip" title="Delete"'
                + ' ref_name="'
                + record.name
                + '" pk="'
                + str(record.pk)
...
class DatasetConsensusTable(tables.Table):
    ...
    project_name = tables.Column(
        "Project Name", orderable=False, empty_values=()
    )  ## when came from projects
    ...
    def render_project_name(self, record):
        ...
        return (
            ...
            if len(record.get_project_name()) > 0
            else self.EMPTY
        )
    ...
    def render_name(self, record):
        ...
        if user.username == record.dataset.owner.username:
            return mark_safe(
                '<a href="#id_remove_modal" id="id_remove_consensus_modal" data-toggle="modal"'
                + ' ref_name="'
                + record.name
                + '" pk="'
                + str(record.pk)
                + '" +\
					" ref_project="'
                + record.name
                + '" data-toggle="tooltip" title="Remove consensus">'
    ...
    def render_technology(self, record):
        ...
        if not record.project_sample is None:
            return (
                ...
                if record.project_sample.sample.is_type_fastq_gz_sequencing()
    ...
    def render_consensus_file(self, record):
        ...
        if not record.project_sample is None:
            return record.project_sample.get_consensus_file_web(
                not default_software.include_consensus(record.project_sample)
            )
        elif not record.consensus is None:
...
class AddDatasetFromCvsFileTableMetadata(tables.Table):
    ...
    def order_is_processed(self, queryset, is_descending):
        queryset = queryset.annotate(is_processed=F("is_processed")).order_by(
