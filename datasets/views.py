...
from constants.constants import Constants, TypeFile, TypePath, FileExtensions
			...
			with open(os.path.join(settings.MEDIA_ROOT, consensus.consensus_fasta.name)) as handle_in:
				for record in SeqIO.parse(handle_in, "fasta"):
					try:
						...
						### if new, create one
						if consensus is None: consensus = Consensus()
						consensus.name = seq_name
						consensus.display_name = seq_name
						consensus.owner = self.request.user
						consensus.is_obsolete = False
						consensus.number_of_locus = 1
						consensus.consensus_fasta_name = utils.clean_name(seq_name + FileExtensions.FILE_FASTA)
