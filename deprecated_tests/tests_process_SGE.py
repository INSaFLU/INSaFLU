...
class Test(TestCase):
    ...
    def test_submitte_job(self):
        ...
        n_count = 0
        while True:
            if process_SGE.is_finished(sge_id):
                break
            time.sleep(1)
            n_count += 1
            if n_count == 100:
        ...
        self.assertTrue(
            os.path.getsize(temp_file) > 80 and os.path.getsize(temp_file) < 100
        )
        self.utils.remove_dir(out_dir)
    ...
    def get_status_running(self, user):
        ...
        """
        data = {}
        data["process_running"] = ProcessControler.objects.filter(
            owner=user, is_finished=False, is_error=False, is_running=True
        ).count()
        data["process_to_run"] = ProcessControler.objects.filter(
            owner=user, is_finished=False, is_error=False, is_running=False
        ).count()
        data["process_to_run_total"] = ProcessControler.objects.filter(
    ...
    def test_create_tree_and_alignments(self):
        ...
        path_destination = os.path.join(
            getattr(settings, "MEDIA_ROOT_TEST", None), ConstantsTestsCase.DIR_PROJECTS
        )
        self.utils.make_path(
            os.path.join(
                getattr(settings, "MEDIA_ROOT_TEST", None),
                ConstantsTestsCase.DIR_PROJECTS,
            )
        )
        cmd = "cp -r {}/{}/* {}".format(
            self.baseDirectory, ConstantsTestsCase.DIR_PROJECTS, path_destination
        )
        os.system(cmd)
        ...
        for vect_file in vect_files:
            self.utils.copy_file(
                vect_file[0], os.path.join(temp_dir, os.path.basename(vect_file[0]))
            )
            self.utils.copy_file(
                vect_file[1], os.path.join(temp_dir, os.path.basename(vect_file[1]))
            )
        ...
        vect_command = [
            "python3 {} collect_global_files --project_id {} --user_id {} --settings fluwebvirus.settings_test".format(
        ...
        try:
            ...
            self.fail("Fail to submit the task")
        self.assertTrue(not sge_id is None)
        ...
        while True:
            ...
            print(dt_running)
            if (
                dt_running["process_to_run"] == 1
                and dt_running["process_running"] == 0
                and dt_running["process_to_run_total"] == 1
            ):
                to_run += 1
            elif (
                dt_running["process_to_run"] == 0
                and dt_running["process_running"] == 1
                and dt_running["process_to_run_total"] == 1
            ):
                running += 1
        ...
        ### samples test
        expected_file_samples = os.path.join(
            self.baseDirectory,
            ConstantsTestsCase.DIR_GLOBAL_PROJECT,
            "insa_flu_sample_output.csv",
        )
        out_file = collect_extra_data.collect_sample_table(
            project, Constants.SEPARATOR_COMMA
        )
        ...
        expected_file_samples = os.path.join(
            self.baseDirectory,
            ConstantsTestsCase.DIR_GLOBAL_PROJECT,
            "insa_flu_sample_output.tsv",
        )
        out_file = collect_extra_data.collect_sample_table(
            project, Constants.SEPARATOR_TAB
        )
        ...
        expected_file_samples = os.path.join(
            self.baseDirectory,
            ConstantsTestsCase.DIR_GLOBAL_PROJECT,
            "insa_flu_variations_snippy.tsv",
        )
        self.assertTrue(filecmp.cmp(out_file, expected_file_samples))
        if os.path.exists(out_file):
            os.unlink(out_file)
        ...
        out_file = collect_extra_data.collect_variations_freebayes(
            project, vect_type_remove
        )
        expected_file_samples = os.path.join(
            self.baseDirectory,
            ConstantsTestsCase.DIR_GLOBAL_PROJECT,
            "insa_flu_variations_freebayes.tsv",
        )
        self.assertTrue(filecmp.cmp(out_file, expected_file_samples))
        if os.path.exists(out_file):
            os.unlink(out_file)
