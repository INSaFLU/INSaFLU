...
class RegistrationForm(UserCreationForm):
    ...
    email = forms.EmailField(
        max_length=254,
        required=True,
        help_text="Required. Inform a valid email address. It is going to be used to valid your account.",
    )
    institution = forms.CharField(
        max_length=100, required=False, help_text="Optional. Affiliation institutional "
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            Div(Div("institution", css_class="col-sm-4"), css_class="row"),
            Div(
                HTML(
                    '<div class="g-recaptcha insa-recaptcha" data-sitekey="{}"></div>'.format(
                        settings.SITE_KEY
                    )
                ),
                css_class="row",
            ...
            ButtonHolder(
                ...
                Button(
                    ...
                    onclick='window.location.href="{}"'.format(reverse("login")),
                ),
            ),
        )
    ...
    def clean(self):
        ...
        try:
            ...
            self.add_error(
                "username",
                ValidationError(
                    _("There's an account with this username."), code="invalid"
                ),
            )
        ...
        try:
            ...
            self.add_error(
                "email",
                ValidationError(
                    _("There's an account with this email."), code="invalid"
                ),
            )
        except User.DoesNotExist:
            pass
...
class ResetPasswordForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.form_method = "POST"
        self.helper.layout = Layout(
            Div(Div("email", css_class="col-sm-4"), css_class="row"),
            Div(
                HTML(
                    '<div class="g-recaptcha insa-recaptcha" data-sitekey="{}"></div>'.format(
                        settings.SITE_KEY
...
class ChangePasswordForm(forms.ModelForm):
    ...
    password2 = forms.CharField(
        label=_("Retype Password"), required=True, widget=forms.PasswordInput
    )
...
class LoginForm(AuthenticationForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if settings.SHOW_LOGIN_ANONYMOUS:
            field_text = [
                ...
                (
                    "login_anonymous",
                    "Demo login.",
                    "Make a demo login. Only can view predefined data.",
                    False,
                ),
            ]
        else:
        ...
        if settings.SHOW_LOGIN_ANONYMOUS:
            self.helper.layout = Layout(
                HTML(
                    "<p><strong>Please, use Firefox, Safari or Chrome browsers for better view experience.</strong></p>"
                ),
                "username",
                "password",
                "login_anonymous",
                ButtonHolder(
                    ...
                    Button(
                        ...
                        "Register",
                        css_class="btn-secondary",
                        onclick='window.location.href="{}"'.format(reverse("register")),
                    ),
                    Button(
                        "reset_password",
                        "Reset password",
            ...
            self.helper.layout = Layout(
                ...
                ButtonHolder(
                    ...
                    Button(
                        "get_message_confirm_email",
                        "Other message to confirm the email.",
                        css_class="btn-secondary",
                        onclick='window.location.href="{}"'.format(
                            reverse("get_message_confirm_email")
                        ),
                    ),
                    Button(
                        "cancel",
