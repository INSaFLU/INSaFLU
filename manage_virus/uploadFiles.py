...
class UploadFiles(object):
    ...
    def get_file_to_upload(self, b_testing):
        ...
        path_to_find = os.path.join(
            ...
            "tests" if b_testing else "",
            self.constants.DIR_TYPE_IDENTIFICATION,
        )
    ...
    def parse_title_name(self, header_name):
        ...
        if len(split_name) > 1:
            dt_return[self.DESCRIPTION] = split_name[1]
        split_name_2 = re.split("~+", split_name[0])
        if len(split_name_2) != 3:
            raise Exception(
                "Error: this seq. name '"
                + split_name
                + "' has a wrong format. Must have 'influenza_type|subtype|lineage~~~<value>~~~<accession> description'"
            )
        split_type = split_name_2[0].split("_")
        if len(split_type) != 2 and (
            not split_type[1].lower() in ConstantsVirus.VECT_ALL_POSSIBLE_TAGS
        ):
            raise Exception(
                "Error: this seq. name '"
                + split_name
    ...
    def __get_type__(self, split_type_name):
        ...
        for tag in ConstantsVirus.VECT_ALL_POSSIBLE_TAGS:
            if split_type[1].lower() == tag.lower():
                return tag
        return ConstantsVirus.SEQ_VIRUS_SUB_TYPE
    ...
    def upload_file(self, version, path):
        ...
        try:
            ...
            uploadFile.abricate_name = self.utils.get_file_name_without_extension(
                base_name
            )
            uploadFile.save()
        ...
        for seq in record_dict:
            ...
            if not seqVirus.kind_type is None:
                ...
                self.logger_production.error(
                    "Fail to upload identification tag: " + seq
                )
    ...
    def uploadIdentifyVirus(self, dict_data, database_name, save=True):
        ...
        dict_data_out = OrderedDict(
            sorted(
                dict_data.items(),
                reverse=True,
                key=lambda k: (
                    k[1][0][ParseOutFiles.COVERAGE],
        ...
        if len(vect_data_out + vect_data_type_and_lineage) > 0:
            for dt_type in vect_data_out + vect_data_type_and_lineage:
                seqVirus = None
                try:
                    seqVirus = SeqVirus.objects.get(
                        name=dt_type.get(ParseOutFiles.GENE),
                        accession=dt_type.get(ParseOutFiles.ACCESSION),
        ...
        for key in dict_data_ordered:
            for dict_info in dict_data_ordered[key]:
                if ParseOutFiles.TYPE in dict_info:
                    ...
                    if (
                        len(split_type) == 2
                        and not (dict_info[ParseOutFiles.GENE] in dict_out_type)
                        and split_type[1].lower() in ConstantsVirus.VECT_SEQ_VIRUS_TYPE
                    ):
        ...
        for key in dict_data_ordered:
            for dict_info in dict_data_ordered[key]:
                if ParseOutFiles.TYPE in dict_info:
                    ...
                    if (
                        type_result.lower() in ConstantsVirus.VECT_SEQ_VIRUS_SUB_TYPE
                        and not dict_info[ParseOutFiles.GENE] in dict_out_gene
                    ):
                        vect_data_out.append(dict_info)
    ...
    def upload_default_references(self, user, b_test):
        ...
        path_to_find = os.path.join(
            getattr(settings, "STATIC_ROOT", None),
            (
                Constants.DIR_TEST_TYPE_REFERENCES
                if b_test
                else Constants.DIR_TYPE_REFERENCES
            ),
        )
        n_upload = 0
        for file in self.utils.get_all_files(path_to_find):
            ...
            try:
                reference = Reference.objects.get(
                    owner=user, is_obsolete=False, is_deleted=False, name__iexact=name
                )
                ...
                for extension in FileExtensions.VECT_ALL_GBK_EXTENSIONS:
                    gbk_file_name = os.path.join(
                        os.path.dirname(file), name + extension
                    )
                    try:
                ...
                if (
                    not gbk_file_name is None
                    and number_of_elements_gbk == number_of_elements
                ):
                ...
                self.utils.from_genbank_to_bed(
                ...
                software.run_genbank2gff3(
                    sz_file_to, reference.get_gff3(TypePath.MEDIA_ROOT)
                )
