...
from managing_files.models import Reference, Primer, Sample, DataSet, VaccineStatus, Project, UploadFiles
...
class PrimerForm(forms.ModelForm):
	...
	def __init__(self, *args, **kwargs):
		...
		field_text= [
			...
			('primer_fasta', 'Primer Set (FASTA)', "Primer file in fasta format.<br>" +\
				"Max total sequence length: {}<br>".format(filesizeformat(settings.MAX_LENGTH_SEQUENCE_TOTAL_FROM_FASTA))  +\
				"Max FASTA file size: {}".format(filesizeformat(settings.MAX_REF_FASTA_FILE)), True),
			('primer_pairs', 'Primer pairs (tabular format)', 
					"""Tabular file containing primer pairs.<br>
					Locus designations must match the ones in the respective fasta file.<br>""" +\
					"Max file size: {}".format(filesizeformat(settings.MAX_REF_GENBANK_FILE)), True),
		]
		for x in field_text:
			self.fields[x[0]].label = x[1]
			self.fields[x[0]].help_text = x[2]
		...
		self.helper.layout = Layout(
			...
			ButtonHolder(
				Submit('save', 'Save', css_class='btn-primary', onclick="this.disabled=true,this.form.submit();"),
				Button('cancel', 'Cancel', css_class='btn-secondary', onclick='window.location.href="{}"'.format(reverse('primer')))
			)
		)
	...
	def clean(self):
		...
		try:
			Primer.objects.get(name__iexact=name, owner=self.request.user, is_deleted=False)
			self.add_error('name', _("This name '" + name +"' already exist in database, please choose other."))
		...
		if (primer_pairs != None and primer_fasta.name == primer_pairs.name):
			self.add_error('primer_fasta', _("Error: both files have the same name. Please, choose different files."))
			self.add_error('primer_pairs', _("Error: both files have the same name. Please, choose different files."))
			return cleaned_data
		...
		try:
			...
			if (number_locus > Constants.MAX_SEQUENCES_FROM_CONTIGS_FASTA):
				self.add_error('primer_fasta', _('Max number of sequences in fasta: {}'.format(Constants.MAX_SEQUENCES_FROM_CONTIGS_FASTA)))
				some_error_in_files = True
			total_length_fasta = self.utils.get_total_length_fasta(primer_fasta_temp_file_name.name)
			if (not some_error_in_files and total_length_fasta > settings.MAX_LENGTH_SEQUENCE_TOTAL_FROM_FASTA):
				some_error_in_files = True
				self.add_error('primer_fasta', _('The max sum length of the sequences in fasta: {}'.format(settings.MAX_LENGTH_SEQUENCE_TOTAL_FROM_FASTA)))
			
			n_seq_name_bigger_than = self.utils.get_number_seqs_names_bigger_than(primer_fasta_temp_file_name.name, Constants.MAX_LENGTH_CONTIGS_SEQ_NAME)
			if (not some_error_in_files and n_seq_name_bigger_than > 0):
				some_error_in_files = True
				if (n_seq_name_bigger_than == 1):
					self.add_error('primer_fasta', _('There is one sequence name length bigger than {0}. The max. length name is {0}.'.format(Constants.MAX_LENGTH_CONTIGS_SEQ_NAME)))
				else:
					self.add_error('primer_fasta', _('There are {0} sequences with name length bigger than {1}. The max. length name is {1}.'.format(n_seq_name_bigger_than, Constants.MAX_LENGTH_CONTIGS_SEQ_NAME)))
