...
class ReferenceForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.attrs["data-validate-reference-url"] = reverse(
        ...
        self.helper.layout = Layout(
            ...
            ButtonHolder(
                ...
                Button(
                    ...
                    onclick='window.location.href="{}"'.format(reverse("references")),
                ),
            ),
        )
        ...
        try:
            ...
            if number_locus > Constants.MAX_SEQUENCES_FROM_FASTA:
                self.add_error(
                    "reference_fasta",
                    _(
                        "Max allow number of sequences in fasta: {}".format(
            ...
            n_seq_name_bigger_than = self.utils.get_number_seqs_names_bigger_than(
                reference_fasta_temp_file_name.name, Constants.MAX_LENGTH_SEQ_NAME
            )
            if not some_error_in_files and n_seq_name_bigger_than > 0:
                ...
                if n_seq_name_bigger_than == 1:
                    self.add_error(
                        "reference_fasta",
                        _(
                            "There is one sequence name length bigger than {0}. The max. length name is {0}. Prokka constrainments.".format(
                    ...
                    self.add_error(
                        "reference_fasta",
                        _(
                            "There are {0} sequences with name length bigger than {1}. The max. length name is {1}. Prokka constrainments.".format(
                                n_seq_name_bigger_than, Constants.MAX_LENGTH_SEQ_NAME
                            )
                        ),
                    )
            ...
            if not self.utils.test_sequences_same_length(
                ...
                self.add_error(
                    "reference_fasta",
                    _(
                        "There are sequences that have not the same length. This produce errors for samtools faidx."
                    ),
                )
                return cleaned_data
    ...
    def __init__(self, *args, **kwargs):
        ...
        field_text = [
            ...
            (
                "date_of_receipt_lab",
                "Lab reception date",
                "Date receipt on the lab.",
                False,
            ),
            (
                "vaccine_status",
                "Vaccine status",
                "Discrimination of vaccination status",
                False,
            ),
            (
                "data_set",
                "Dataset",
                "Specific dataset (useful for grouping sets of samples)",
                False,
            ),
            ...
            (
                "type_of_fastq",
                "Technology",
                "Sequencing technology<br/> -1 : infer automatically<br/> 0 : Illumina / Ion Torrent<br/> 1 : ONT",
                False,
            ),
        ]
        for x in field_text:
        ...
        self.helper.layout = Layout(
            ...
            ButtonHolder(
                ...
                Button(
                    ...
                    "Cancel",
                    css_class="btn-secondary",
                    onclick='window.location.href="{}"'.format(reverse("samples")),
                ),
            ),
        )
    ...
    def clean(self):
        ...
        try:
            ...
            if lng != None and (lng > 180 or lng < -180):
                self.add_error(
                    "lng", _("Longitude must have values between +180 and -180.")
                )
            ...
            ## testing fastq
            fastaq_temp_file_name = NamedTemporaryFile(
            ...
            if (
                ...
                self.add_error(
                    "date_of_collection", _("Error, the Collection date is null.")
                )
            elif like_dates == "date_of_receipt_lab" and date_of_receipt_lab == None:
                self.add_error(
                    "date_of_receipt_lab", _("Error, the Lab Receipt date is null.")
                )

            try:
                if not self.utils.is_gzip(path_name_1.name):
                ...
                ## check path_name_2
                if not path_name_2 is None and not self.utils.is_gzip(path_name_2.name):
                    self.add_error(
                        "path_name_2", "File need to have suffix '.fastq.gz'/'.fq.gz'"
                    )
                return cleaned_data
            ...
            if path_name_2 != None:
                ...
                try:
                    if not self.utils.is_gzip(path_name_2.name):
                        raise Exception("File need to have suffix '.fastq.gz'/'.fq.gz'")
                    (is_fastq, type_of_fastq) = self.utils.is_fastq_gz(
                        fastaq_temp_file_name_2.name
                    )
                    ...
                    if type_of_fastq != Constants.FORMAT_FASTQ_illumina:
                        ...
                        self.add_error(
                            "path_name_2",
                            _(
                                "Only illumina files can be added to the second file name."
                            ),
                        )
                        return cleaned_data
            ...
            self.logger_production.error(
                "New Sample: {}  Path name2: {} Can't reach second file".format(
                    name, path_name_1
                )
            )
        ...
        if type_of_fastq not in [-1, 0, 1]:
            self.add_error(
                "type_of_fastq",
                _("Error: Technology must be -1 (automatic), 0 (Illumina) or 1 (ONT)."),
            )
            return cleaned_data

        if (type_of_fastq == 1) and (self.cleaned_data.get("path_name_2") != None):
            self.add_error(
                "type_of_fastq", _("Error: ONT data cannot have second fastq")
            )
            return cleaned_data
...
## to put both inline
ReferenceProjectFormSet = inlineformset_factory(
    Reference, Project, form=ReferenceProjectForm, extra=1
)
...
class SamplesUploadDescriptionForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        field_text = [
            (
                "path_name",
                "File name",
                '"csv" or "tsv" file with several samples to upload fastq.gz files later',
                True,
            ),
        ]
        for x in field_text:
        ...
        self.helper.layout = Layout(
            ...
            # HTML('<p> </p>'),
            HTML(
                '<div class="alert alert-dark"> <a href="'
                + mark_safe(
                    os.path.join(
                        getattr(settings, "STATIC_URL", None),
                        Constants.DIR_TEMPLATE_INPUT,
                        Constants.FILE_TEMPLATE_INPUT_tsv,
                    )
                )
                + '" download> <span> <i class="fa fa-download"></i></span> Template file \'tsv\'</a> </div>'
            ),
            HTML("<p> </p>"),
            # HTML('<div class="alert alert-dark"> <a href="' + mark_safe(os.path.join(getattr(settings, "STATIC_URL", None), Constants.DIR_TEMPLATE_INPUT,\
            ...
            HTML(
                ...
                + mark_safe(
                    os.path.join(
                        getattr(settings, "STATIC_URL", None),
                        Constants.DIR_TEMPLATE_INPUT,
                        Constants.FILE_TEMPLATE_INPUT_data_tsv,
                    )
                )
                + '" download> <span> <i class="fa fa-download"></i></span> Example Template file \'tsv\'</a> </div>'
            ),
            HTML("<p> </p>"),
    ...
    def clean(self):
        ...
        temp_file_name = NamedTemporaryFile(
            prefix="flu_fq_", suffix=".csv", delete=False
        )
        ...
        parse_in_files.parse_sample_files(
            ...
            ParseInFiles.STATE_READ_only_detect_errors,
        )
        ...
        if parse_in_files.get_errors().has_errors():
            ...
            self.error_in_file = str(
                parse_in_files.get_errors()
            )  ## pass error_file kwargs for context
        else:
    ...
    def __init__(self, *args, **kwargs):
        ...
        field_text = [
            (
                "path_name",
                "File name",
                '"csv" or "tsv" file with metadata to update existing samples.',
                True,
            ),
        ]
        for x in field_text:
        ...
        self.helper.layout = Layout(
            ...
            HTML(
                ...
                + '" download> <span> <i class="fa fa-download"></i></span> Template Metadata file \'csv\'</a> </div>'
            ),
            HTML("<p> </p>"),
            HTML(
                '<div class="alert alert-dark"> <a href="'
                ...
                + '" download> <span> <i class="fa fa-download"></i></span> Template Metadata file \'tsv\'</a> </div>'
            ),
            HTML("<p> </p>"),
            HTML(
                '<div class="alert alert-dark"> <a href="'
                + mark_safe(
                    os.path.join(
                        getattr(settings, "STATIC_URL", None),
                        Constants.DIR_TEMPLATE_INPUT,
                        Constants.FILE_TEMPLATE_INPUT_METADATA_data_csv,
                    )
                )
                + '" download> <span> <i class="fa fa-download"></i></span> Example Template Metadata file \'csv\'</a> </div>'
            ),
            HTML("<p> </p>"),
            HTML(
                '<div class="alert alert-dark"> <a href="'
                + mark_safe(
                    os.path.join(
                        getattr(settings, "STATIC_URL", None),
                        Constants.DIR_TEMPLATE_INPUT,
                        Constants.FILE_TEMPLATE_INPUT_METADATA_data_tsv,
                    )
                )
                + '" download> <span> <i class="fa fa-download"></i></span> Example Template Metadata file \'tsv\'</a> </div>'
            ),
            HTML("<p> </p>"),
            Div("path_name", css_class="col-lm-3"),
            HTML("<p> </p>"),
            ButtonHolder(
                ...
                Button(
                    ...
                    css_class="btn-secondary",
                    onclick='window.location.href="{}"'.format(
                        reverse("sample-update-metadata")
                    ),
                ),
            ),
        )
...
class SamplesUploadMultipleFastqForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        number_files = UploadFiles.objects.filter(
            file_name__iexact=os.path.basename(path_name.name),
            owner=self.request.user,
            is_processed=False,
            is_deleted=False,
