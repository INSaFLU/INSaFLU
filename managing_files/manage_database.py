...
class ManageDatabase(object):
    ...
    def set_reference_metakey(
        self, reference, owner, meta_key_name, value, description
    ):
        """
        save a meta key
        """
    ...
    def get_reference_metakey(self, reference, meta_key_name, value):
        ...
        try:
            if value == None:
                return MetaKeyReference.objects.filter(
                    reference__id=reference.id, meta_tag__name=meta_key_name
                ).order_by("-creation_date")
            return MetaKeyReference.objects.get(
                reference__id=reference.id, meta_tag__name=meta_key_name, value=value
            )
    ...
    def get_reference_metakey_last(self, reference, meta_key_name, value):
        ...
        try:
            if value == None:
                query_set = MetaKeyReference.objects.filter(
                    reference__id=reference.id, meta_tag__name=meta_key_name
                ).order_by("-creation_date")
            else:
                query_set = MetaKeyReference.objects.filter(
                    reference__id=reference.id,
                    meta_tag__name=meta_key_name,
                    value=value,
                ).order_by("-creation_date")
    ...
    def get_sample_metakey(self, sample, meta_key_name, value):
        ...
        try:
            if value == None:
                return MetaKeySample.objects.filter(
                    sample__id=sample.id, meta_tag__name=meta_key_name
                ).order_by("-creation_date")
            return MetaKeySample.objects.get(
                sample__id=sample.id, meta_tag__name=meta_key_name, value=value
            )
    ...
    def is_sample_wating_fastq_file(self, sample):
        ...
        meta_sample_queue = self.get_sample_metakey_last(
            sample,
            MetaKeyAndValue.META_KEY_Queue_TaskID,
            MetaKeyAndValue.META_VALUE_Queue,
        )
        if meta_sample_queue is None:
            return True
        return False
    ...
    def is_sample_processing_step(self, sample):
        ...
        ### try to find errors
        number_errors = (
            MetaKeySample.objects.filter(
                sample=sample,
                value=MetaKeyAndValue.META_VALUE_Error,
                creation_date__gt=meta_sample_queue.creation_date,
            )
            .order_by("-creation_date")
            .count()
        )
        ...
        meta_sample_queue_success = self.get_sample_metakey_last(
            sample,
            MetaKeyAndValue.META_KEY_Queue_TaskID,
            MetaKeyAndValue.META_VALUE_Success,
        )
        if meta_sample_queue_success is None:
            return True
        if (
            meta_sample_queue_success.creation_date > meta_sample_queue.creation_date
            and meta_sample_queue_success.description == meta_sample_queue.description
        ):
            return False
        return True
    ...
    def get_sample_metakey_last(self, sample, meta_key_name, value):
        ...
        try:
            if value == None:
                query_set = MetaKeySample.objects.filter(
                    sample__id=sample.id, meta_tag__name=meta_key_name
                ).order_by("-creation_date")
            else:
                query_set = MetaKeySample.objects.filter(
    ...
    def remove_sample_start_metakey(self, sample, meta_key_name):
        ...
        """
        return MetaKeySample.objects.filter(
            sample__id=sample.id, meta_tag__name__startswith=meta_key_name
        ).delete()
    ...
    def update_project_metakey(self, project, owner, meta_key_name, value, description):
        ...
        metaKeyProject_list = MetaKeyProject.objects.filter(
            project=project, meta_tag=metaKey, owner=owner
        )
        if metaKeyProject_list.count() == 0:
            return self.set_project_metakey(
                project, owner, meta_key_name, value, description
            )
        else:
    ...
    def get_project_metakey(self, project, meta_key_name, value):
        ...
        try:
            if value == None:
                return MetaKeyProject.objects.filter(
                    project__id=project.id, meta_tag__name=meta_key_name
                ).order_by("-creation_date")
            return MetaKeyProject.objects.get(
                project__id=project.id, meta_tag__name=meta_key_name, value=value
            )
    ...
    def get_project_metakey_last(self, project, meta_key_name, value):
        ...
        try:
            if value == None:
                query_set = MetaKeyProject.objects.filter(
                    project__id=project.id, meta_tag__name=meta_key_name
                ).order_by("-creation_date")
            else:
                query_set = MetaKeyProject.objects.filter(
    ...
    def is_project_processing_step(self, project, tag_key):
        ...
        meta_project_queue = self.get_project_metakey_last(
            project, tag_key, MetaKeyAndValue.META_VALUE_Queue
        )
        if meta_project_queue is None:
            return True
        ...
        number_errors = (
            MetaKeySample.objects.filter(
                project=project,
                value=MetaKeyAndValue.META_VALUE_Error,
                creation_date__gt=meta_project_queue.creation_date,
            )
            .order_by("-creation_date")
            .count()
        )
        ...
        ### try to find queue_success
        meta_project_queue_success = self.get_project_metakey_last(
            project, tag_key, MetaKeyAndValue.META_VALUE_Success
        )
        if meta_project_queue_success is None:
            return True
        if (
            meta_project_queue_success.creation_date > meta_project_queue.creation_date
            and meta_project_queue_success.description == meta_project_queue.description
        ):
            return False
        return True

    def set_project_sample_metakey(
        self, project_sample, owner, meta_key_name, value, description
    ):
        """
        save a meta key
        """
    ...
    def get_project_sample_metakey(self, project_sample, meta_key_name, value):
        ...
        try:
            if value == None:
                return MetaKeyProjectSample.objects.filter(
                    project_sample__id=project_sample.id, meta_tag__name=meta_key_name
                ).order_by("-creation_date")
            return MetaKeyProjectSample.objects.get(
                project_sample__id=project_sample.id,
                meta_tag__name=meta_key_name,
                value=value,
            )
    ...
    def get_project_sample_metakey_last(self, project_sample, meta_key_name, value):
        ...
        try:
            if value == None:
                query_set = MetaKeyProjectSample.objects.filter(
                    project_sample__id=project_sample.id, meta_tag__name=meta_key_name
                ).order_by("-creation_date")
            else:
                query_set = MetaKeyProjectSample.objects.filter(
    ...
    def get_project_sample_starts_with_metakey(self, project_sample, meta_key_name):
        ...
        try:
            return list(
                MetaKeyProjectSample.objects.filter(
                    project_sample__id=project_sample.id,
                    meta_tag__name__startswith=meta_key_name,
                ).order_by("-creation_date")
            )
        except MetaKeyProject.DoesNotExist:
            return []
    ...
    def is_sample_downsized(self, sample):
        ...
        if not meta_sample_queue is None:
            return True
        return False
    ...
    def get_max_length_label(self, project, user, b_calculate_again):
        ...
        if not b_calculate_again:
            meta_data = self.get_project_metakey_last(
                project,
                MetaKeyAndValue.META_KEY_Project_max_sample_length,
                MetaKeyAndValue.META_VALUE_Success,
            )
            if meta_data != None:
        ...
        for project_sample in project.project_samples.all():
            ...
            if project_sample.is_error:
                continue
            if len(project_sample.sample.name) > n_max_value:
                n_max_value = len(project_sample.sample.name)
        ## calculate for the reference too
        if len(project.reference.display_name) > n_max_value:
            n_max_value = len(project.reference.display_name)

        meta_data = self.update_project_metakey(
            project,
            user,
            MetaKeyAndValue.META_KEY_Project_max_sample_length,
            MetaKeyAndValue.META_VALUE_Success,
    ...
    def percentils_refresh(self, user):
        ...
        if (
            CountVariations.objects.count()
            < TagNamesConstants.MINIMUM_NUMBER_TO_COUNT_STATISTICS
        ):
            return
    ...
    def set_percentis_alert(self, project_sample, user, count_hits, percentil_name):
        ...
        if percentil_value != None:
            ...
            if (
                percentil_name.endswith(TagNamesConstants.TAG_PERCENTIL_VAR_TOTAL)
                and count_hits.get_total() > percentil_value
            ):
                b_percentil_fault = True
            elif (
                percentil_name.endswith(TagNamesConstants.TAG_PERCENTIL_VAR_50)
                and count_hits.get_hits_less_50() > percentil_value
            ):
                b_percentil_fault = True
            elif (
                percentil_name.endswith(TagNamesConstants.TAG_PERCENTIL_VAR_50_90)
                and count_hits.get_hits_50_90() > percentil_value
            ):
                b_percentil_fault = True
            ...
            if b_percentil_fault:
                ...
                project_sample.save()
                self.set_project_sample_metakey(
                    project_sample,
                    user,
                    MetaKeyAndValue.META_KEY_ALERT_COUNT_VAR,
                    MetaKeyAndValue.META_VALUE_Error,
                    "Warning, this sample has a total of variations '{}' bigger than the percentile {} of all database".format(
                        count_hits.get_total(),
                        int(
                            tagNamesConstants.get_number_percentil_from_tag(
                                percentil_name
                            )
                        ),
                    ),
    ...
    def get_percentil_value(self, percentil_name):
        ...
        tagNamesConstants = TagNamesConstants()
        percentil_float = tagNamesConstants.get_number_percentil_from_tag(
            percentil_name
        )
        ...
        if tagNamesConstants.is_which_var(
            percentil_name, TagNamesConstants.TAG_PERCENTIL_VAR_TOTAL
        ):
            cursor.execute(
                "select avg(total) from (select total, ntile(100) over (order by total), cume_dist() OVER (ORDER BY total)"
                + " as percentile from managing_files_countvariations) ss where ntile = {};".format(
                    percentil_float
                )
            )
        elif tagNamesConstants.is_which_var(
            percentil_name, TagNamesConstants.TAG_PERCENTIL_VAR_50
        ):
            cursor.execute(
                "select avg(var_less_50) from (select var_less_50, ntile(100) over (order by var_less_50), cume_dist() OVER (ORDER BY var_less_50)"
                + " as percentile from managing_files_countvariations) ss where ntile = {};".format(
                    percentil_float
                )
            )
        elif tagNamesConstants.is_which_var(
            percentil_name, TagNamesConstants.TAG_PERCENTIL_VAR_50_90
        ):
            cursor.execute(
                "select avg(var_bigger_50) from (select var_bigger_50, ntile(100) over (order by var_bigger_50), cume_dist() OVER (ORDER BY var_bigger_50)"
                + " as percentile from managing_files_countvariations) ss where ntile = {};".format(
                    percentil_float
                )
            )
        row = cursor.fetchall()
        if row != None and len(row) > 0 and len(row[0]) > 0:
            return int(row[0][0])
        return None
