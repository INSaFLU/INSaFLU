...
class Command(BaseCommand):
	'''
	classdocs
	Ex: python3 manage.py collect_update_pangolin_lineage --project_id 11 --user_id 1
	...
	def handle(self, *args, **options):
		...
		self.logger_production.info("Starting update pangolin for project_id: " + str(project_id))
		self.logger_debug.info("Starting update pangolin for project_id: " + str(project_id))

		try:
			project = Project.objects.get(pk=project_id)
			if (user_id == None): user = project.owner
			else: user = User.objects.get(pk=user_id)
			collect_extra_data.collect_update_pangolin_lineage(project, user)
