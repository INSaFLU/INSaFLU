...
from constants.constants import TypeFile, TypePath
from constants.software_names import SoftwareNames
from managing_files.models import Project, ProjectSample, Reference, Sample, UploadFiles
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--delete_trimmomatic_nanofilt_fastq",
            type=str2bool,
            nargs="?",
            const=False,
            default=False,
    ...
    def handle(self, *args, **options):
        ...
        self.out_message(
            "\nMode - {}\nDelete Original fastq files: {}\n".format(
                (
                    ...
                    else "Identify files and remove them."
                ),
                str(delete_original_fastq),
            )
            + "Delete Trimmomatic/Nanofilt fastq files: {}".format(
                str(delete_trimmomatic_nanofilt_fastq)
            ),
            False,
        )
        ### dates
        if (
            options["from_date"] != NO_DATE
            and options["end_date"] != NO_DATE
            and options["end_date"] <= options["from_date"]
        ):
            self.out_message("\n'From Date' must be bigger to 'End Date'", True)
            sys.exit(1)

        if options["from_date"] != NO_DATE:
            self.out_message("From date: {}".format(options["from_date"]), False)
        if options["end_date"] != NO_DATE:
            self.out_message("End date: {}".format(options["end_date"]), False)
        if options["from_date"] == NO_DATE and options["end_date"] == NO_DATE:
            self.out_message(
                "Remove files after {} days passed from been removed in web site".format(
                    self.REMOVE_FILES_AFTER_DAYS
                )
                + " by the users.",
                False,
            )
        ...
        if len(logins_to_pass) > 0:
            lst_accounts_to_pass = logins_to_pass.strip().split(",")

        ## logins to process
        logins_to_process = options["login_to_process"]
        lst_accounts_to_process = []
        if len(logins_to_process) > 0:
            lst_accounts_to_process = logins_to_process.strip().split(",")

        if len(lst_accounts_to_process) > 0 and len(lst_accounts_to_pass) > 0:
            self.out_message(
                "\nYou can not use '--login_to_process' and '--login_to_pass' simultaneously.",
                True,
            )
            sys.exit(1)
        ...
        if len(lst_accounts_to_pass) > 0:
            self.out_message(
                "Login to pass: {}".format(",".join(lst_accounts_to_pass)), False
            )
        if len(logins_to_process) > 0:
            self.out_message(
                "Login to process: {}".format(",".join(lst_accounts_to_process)), False
            )
        if options["end_date"] != NO_DATE and options["from_date"] != NO_DATE:
            self.out_message(
                "Warning: the parameter 'remove_files_after_days = {}' will be deactivated.".format(
                    self.REMOVE_FILES_AFTER_DAYS
                ),
                False,
            )
        ...
        if options["end_date"] != NO_DATE:
            self.has_end_date = (
                True  ###if true the parameter 'remove_files_after_days' will be disable
            )
        ...
        ### delete REFERENCES
        total_files_downloaded = self.delete_references(
            only_identify_files, lst_accounts_to_pass, lst_accounts_to_process
        )

        ### delete SAMPLES
        total_files_downloaded += self.delete_all_samples(
            only_identify_files, lst_accounts_to_pass, lst_accounts_to_process
        )

        ### delete original fastq SAMPLES
        if delete_original_fastq:
            total_files_downloaded += self.delete_original_fastq_samples(
                only_identify_files, lst_accounts_to_pass, lst_accounts_to_process
            )

        if delete_trimmomatic_nanofilt_fastq:
            total_files_downloaded += self.delete_trimmomatic_nanofilt_fastq_samples(
                only_identify_files, lst_accounts_to_pass, lst_accounts_to_process
            )

        ### remove fastq.gz files not attached and already deleted
        total_files_downloaded += (
            self.delete_upload_deleted_files_by_batch_and_not_attached(
                only_identify_files, lst_accounts_to_pass, lst_accounts_to_process
            )
        )

        ### remove project_samples "sample" is the sample and project sample was deleted
        total_files_downloaded += self.delete_project_samples(
            only_identify_files, lst_accounts_to_pass, lst_accounts_to_process
        )

        if only_identify_files:
            if total_files_downloaded == 0:
                self.out_message("No file were identified", False)
            else:
                self.out_message(
                    "Files identified: {}".format(total_files_downloaded), False
                )
        else:
            if total_files_downloaded == 0:
                self.out_message("No files were removed", False)
            else:
                self.out_message(
                    "Files removed: {}".format(total_files_downloaded), False
                )

    def out_message(self, message, b_error):
        if b_error:
            self.logger_remove_files.error(message.strip())
        else:
            self.logger_remove_files.info(message.strip())
        self.stdout.write(message)

    def test_existence_accounts(self, lst_accounts_to_pass, sz_message=""):
        if len(lst_accounts_to_pass) > 0:
            self.out_message(
                "\nTesting existence of the accounts {}- {}".format(
                    sz_message, ",".join(lst_accounts_to_pass)
                ),
                False,
            )
            lst_not_exist = []
    ...
    def is_out_date_to_delete(self, date_to_evaluate):
        ...
        if not date_to_evaluate is None and self.has_end_date and self.has_from_date:
            if self.from_date > date_to_evaluate or self.end_date < date_to_evaluate:
                return True
            return False
        ## only from_date not null
        if (
            not date_to_evaluate is None
            and self.has_from_date
            and self.from_date > date_to_evaluate
        ):
            return True
        ## only end_date not null
        if (
            not date_to_evaluate is None
            and self.has_end_date
            and self.end_date < date_to_evaluate
        ):
            return True

        if date_to_evaluate != None:
            removed_days = int(
                divmod(
                    (datetime.datetime.now() - date_to_evaluate).total_seconds(), 86400
                )[0]
            )
        else:
            removed_days = (
                100000  ## big number, older versions doesn't have this table field
            )
    ...
    def delete_references(
        self, only_identify_files, lst_accounts_to_pass, lst_accounts_to_process
    ):
        """
        ...
        references = Reference.objects.all().filter(
            is_deleted=True, is_deleted_in_file_system=False
        )
        count = 0
        for reference in references:
            ### test the owners
            if reference.owner.username in lst_accounts_to_pass:
                continue
            if (
                len(lst_accounts_to_process) > 0
                and not reference.owner.username in lst_accounts_to_process
            ):
                continue
            ...
            if (
                ...
                try:
                    ...
                    files_to_remove.append(
                        reference.get_reference_fasta_index(TypePath.MEDIA_ROOT)
                    )
                    files_to_remove.append(
                        reference.get_reference_bed(TypePath.MEDIA_ROOT)
                    )
                    files_to_remove.append(
                        reference.get_reference_bed_index(TypePath.MEDIA_ROOT)
                    )
        ...
        if only_identify_files:
            if count == 0:
                self.out_message("None references were identified", False)
            else:
                self.out_message("Files identified: {}".format(count), False)
        else:
            if count == 0:
                self.out_message("None references were removed", False)
            else:
    ...
    def delete_all_samples(
        ...
        try:
            ...
            samples_deleted = self._delete_all_samples(
                samples,
                only_identify_files,
                lst_accounts_to_pass,
                lst_accounts_to_process,
        ...
        date_remove = datetime.datetime.now() - dateutil.relativedelta.relativedelta(
            months=1
        )
        samples = Sample.objects.all().filter(
            is_deleted=True,
            is_deleted_in_file_system=False,
            is_valid_1=True,
            is_ready_for_projects=False,
            creation_date__lte=date_remove,
        )
        samples_deleted += self._delete_all_samples(
            samples, only_identify_files, lst_accounts_to_pass, lst_accounts_to_process
        )

        if samples_deleted == 0:
            if only_identify_files:
                self.out_message("None samples were removed.", False)
            else:
                self.out_message("None samples were removed", False)
        self.out_message("### END  Samples Deleted\n", False)
        return samples_deleted

    def _delete_all_samples(
        self,
        samples,
        only_identify_files,
        lst_accounts_to_pass,
        ...
        for sample in samples:
            ### test users
            if sample.owner.username in lst_accounts_to_pass:
                continue
            if (
                len(lst_accounts_to_process) > 0
                and not sample.owner.username in lst_accounts_to_process
            ):
                continue
            ...
            if (
                ProjectSample.objects.all()
                .filter(sample=sample, is_deleted=False)
                .count()
                == 0
            ):
                files_removed = []
                ...
                try:
                    ...
                    if sample.is_type_fastq_gz_sequencing():  ### illumina
                        ...
                        files_to_remove.append(
                            sample.get_abricate_output(TypePath.MEDIA_ROOT)
                        )
                    else:  ### minion
                        if not sample.is_original_fastq_removed():
                            files_to_remove.append(
                            ...
                            files_to_remove.append(
                                sample.get_rabbitQC_output(TypePath.MEDIA_ROOT)
                            )
                            original_file_not_removed = True
                        files_to_remove.append(
                    ...
                    if only_identify_files:
                        ...
                        if len(files_removed) > 0:
                            ...
                            sample.date_deleted_processed_fastq = (
                            ...
                            if original_file_not_removed:
                                upload_files = UploadFiles.objects.all().filter(
                                    samples__id=sample.pk,
        ...
        if only_identify_files:
            if count == 0:
                self.out_message("None samples were identified", False)
            else:
                self.out_message("Files identified: {}".format(count), False)
        else:
            if count == 0:
    ...
    def delete_original_fastq_samples(
        self, only_identify_files, lst_accounts_to_pass, lst_accounts_to_process
    ):
        """
        ...
        samples_deleted = self._delete_original_fastq(
            samples, only_identify_files, lst_accounts_to_pass, lst_accounts_to_process
        )

        if samples_deleted == 0:
            if only_identify_files:
                self.out_message("None original fastq.gz were identified", False)
            else:
                self.out_message("None original fastq.gz were removed", False)
        self.out_message("### END  Original fastq files\n", False)
        return samples_deleted

    def _delete_original_fastq(
        self,
        samples,
        only_identify_files,
        lst_accounts_to_pass,
        ...
        for sample in samples:
            ...
            if sample.is_original_fastq_removed():
                continue
            if sample.owner.username in lst_accounts_to_pass:
                continue
            if (
    ...
    def delete_trimmomatic_nanofilt_fastq_samples(
        self, only_identify_files, lst_accounts_to_pass, lst_accounts_to_process
    ):
        """
        ...
        samples_deleted = self._delete_processed_fastq(
            samples, only_identify_files, lst_accounts_to_pass, lst_accounts_to_process
        )
        ...
        if samples_deleted == 0:
            if only_identify_files:
                self.out_message(
                    "None trimmomatic-nanofilt-fastq were identified", False
                )
            else:
                self.out_message("None trimmomatic-nanofilt-fastq were removed", False)
        self.out_message("### END trimmomatic-nanofilt-fastq files\n", False)
        return samples_deleted

    def _delete_processed_fastq(
        self,
        samples,
        only_identify_files,
        lst_accounts_to_pass,
        ...
        for sample in samples:
            ...
            if sample.is_processed_fastq_deleted():
                continue
            if sample.owner.username in lst_accounts_to_pass:
                continue
            if (
    ...
    def delete_upload_deleted_files_by_batch_and_not_attached(
        self, only_identify_files, lst_accounts_to_pass, lst_accounts_to_process
    ):
        """
        ...
        for upload_file in upload_files:
            ...
            ### test the owner
            if upload_file.owner.username in lst_accounts_to_pass:
                continue
            if (
                len(lst_accounts_to_process) > 0
                and not upload_file.owner.username in lst_accounts_to_process
            ):
                continue
            ...
            try:
                files_to_remove.append(
                    upload_file.get_path_to_file(TypePath.MEDIA_ROOT)
                )
        ...
        if only_identify_files:
            if count == 0:
                self.out_message(
                    "None Uploaded fastq files by batch were identified", False
                )
            else:
                self.out_message("Files identified: {}".format(count), False)
        else:
    ...
    def delete_project_samples(
        ...
        project_samples = ProjectSample.objects.all().filter(
            is_deleted=True, is_deleted_in_file_system=False, sample__is_deleted=True
        )
        ...
        for project_sample in project_samples:
            files_removed = []
            files_to_remove = []
            software_illumina_mdcg = software_names.get_snippy_name()
            ...
            if (
                len(lst_accounts_to_process) > 0
                and not project_sample.project.owner.username in lst_accounts_to_process
            ):
                continue
            ...
            try:
                ...
                for type_file in software.get_vect_type_files_to_copy(
                    ...
                    files_to_remove.append(
                        project_sample.get_file_output(
                            TypePath.MEDIA_ROOT, type_file, software_illumina_mdcg
                        )
                    )
                for type_file in software.get_vect_type_files_to_copy(
                ...
                for type_file in software.get_vect_type_files_to_copy(
                    software_names.get_medaka_name()
                ):
                    files_to_remove.append(
                        project_sample.get_file_output(
                            TypePath.MEDIA_ROOT,
