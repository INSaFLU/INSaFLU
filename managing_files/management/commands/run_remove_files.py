...
from constants.constants import TypePath, TypeFile, FileType
...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		self.out_message("\nMode - {}".format("Only identify files" if only_identify_files else "Identify files and remove them."), False)
		
		### delete REFERENCES
		total_files_downloaded = self.delete_references(only_identify_files)

		### delete SAMPLES
		total_files_downloaded += self.delete_all_samples(only_identify_files)
		
		### delete original fastq SAMPLES
		total_files_downloaded += self.delete_original_fastq_samples(only_identify_files)
		...
		total_files_downloaded += self.delete_project_samples(only_identify_files)
	...
	def delete_references(self, only_identify_files):
		"""
	...
	def delete_all_samples(self, only_identify_files):
		"""
		...
		try:
			...
			samples_deleted = self._delete_all_samples(samples, only_identify_files)
	...
	def _delete_all_samples(self, samples, only_identify_files):
		"""
	...
	def delete_original_fastq_samples(self, only_identify_files):
		"""
		delete all samples deleted by the users 
		"""
		...
		samples_deleted = self._delete_original_fastq(samples, only_identify_files)
	...
	def _delete_original_fastq(self, samples, only_identify_files):
		"""
		Delete all samples by array
		"""
		count = 0
	...
	def delete_upload_deleted_files_by_batch_and_not_attached(self, only_identify_files):
		"""
