...
class Sample(models.Model):
	...
	def get_fastqc_output(self, type_path, b_first_file):
		...
		"""
		if (not b_first_file and not self.exist_file_2()): return None
		return os.path.join(self.__get_path__(type_path, b_first_file), self.file_name_1.replace(FileExtensions.FILE_FASTQ_GZ, "_fastqc.html").replace(FileExtensions.FILE_FQ_GZ, "_fastqc.html") \
			 if b_first_file else self.file_name_2.replace(FileExtensions.FILE_FASTQ_GZ, "_fastqc.html").replace(FileExtensions.FILE_FQ_GZ, "_fastqc.html"))
	...
	def get_rabbitQC_output(self, type_path):
		...
		"""
		b_first_file = True
		return os.path.join(self.__get_path__(type_path, b_first_file), self.file_name_1.replace(FileExtensions.FILE_FASTQ_GZ, "_rabbitQC.html").replace(FileExtensions.FILE_FQ_GZ, "_rabbitQC.html"))
