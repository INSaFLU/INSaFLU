...
class PhyloTreeManager:
    ...
    def inner_node_children_dict_get(self):
        ...
        inner_node_children_dict= {node: self.get_node_children(node) for node in inner_nodes}

        inner_node_children_dict= {node: [child.name for child in children] for node, children in inner_node_children_dict.items()}
    ...
    def inner_node_leaf_dict_get(self):   
        ...
        """
        inner_nodes= self.inner_nodes_get()
        inner_node_leaf_dict= {node: self.get_node_leaves(node) for node in inner_nodes}

        inner_node_leaf_dict= {node: [leaf.name for leaf in leaves] for node, leaves in inner_node_leaf_dict.items()}
    ...
    def clades_get_children_clades(self, private_clades= []):
        ...
        if private_clades:
            inner_node_clades= {clade: nodes for clade, nodes in inner_node_clades.items() if clade in private_clades}
    ...
    def clades_get_leaves_clades(self, private_clades= []):
        ...
        if private_clades:
            inner_node_leaf_dict= {clade: nodes for clade, nodes in inner_node_leaf_dict.items() if clade in private_clades}

        inner_node_leaf_dict= {node: [leaf for leaf in leaves if leaf not in inner_nodes] for node, leaves in inner_node_leaf_dict.items()}
        inner_node_leaf_dict= {node: [leaf.name for leaf in leaves] for node, leaves in inner_node_leaf_dict.items()}
        return inner_node_leaf_dict
