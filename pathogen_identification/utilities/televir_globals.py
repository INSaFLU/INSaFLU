...
def get_prinseq_software(username:str, project_name) -> PrinseqParams:
    ...
    prinseq_params= Parameter.objects.filter(software=prinseq, televir_project__name=project_name)
    if prinseq_params.count() == 0:
        prinseq_params= Parameter.objects.filter(software=prinseq, televir_project__name=None)
    if prinseq_params.count() == 0:
        raise Exception(f"Prinseq software parameters not found for user {username} and project {project_name}")
    entropy_threshold= 0
    dust_threshold= 0
    ...
    prinseq= PrinseqParams(entropy_threshold=entropy_threshold, dust_threshold=dust_threshold, is_to_run=prinseq.is_to_run)
