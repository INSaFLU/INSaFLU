...
from constants.software_names import Constants, SoftwareNames
...
class testsDefaultSoftwares(TestCase):
    ...
    def test_software_obsolete(self):
        ...
        try:
            user = User.objects.get(
                username=ConstantsTestsCase.TEST_USER_NAME + " set obsolete"
            )
        ...
        self.assertEqual(
            SoftwareNames.INSAFLU_PARAMETER_MASK_CONSENSUS_name, vect_software[4]
        )
        ...
        self.assertEqual(
            "-q 10 -l 50 --headcrop 70 --tailcrop 70",
            default_software.get_nanofilt_parameters(user),
        )
        self.assertEqual(
            "--minid 70 --mincov 30",
            default_software.get_abricate_parameters(
        ...
        query_set = Software.objects.filter(
            name=SoftwareNames.SOFTWARE_TRIMMOMATIC_name,
        ...
        try:
            software = Software.objects.get(
                ...
                owner=user,
                type_of_use=Software.TYPE_OF_USE_global,
                version_parameters=default_parameters.get_software_parameters_version(
                    SoftwareNames.SOFTWARE_TRIMMOMATIC_name
                ),
            )
            self.assertEqual(
                default_parameters.get_software_parameters_version(
                    SoftwareNames.SOFTWARE_TRIMMOMATIC_name
                ),
    ...
    def test_default_software(self):
        ...
        self.assertEqual(
            "-q 10 -l 50 --headcrop 70 --tailcrop 70",
            default_software.get_parameters(
                SoftwareNames.SOFTWARE_NanoFilt_name,
                user,
                name_extended=SoftwareNames.SOFTWARE_NanoFilt_name_extended,
            ),
        )
        self.assertEqual(
            "Threshold:70",
            default_software.get_mask_consensus_threshold_parameters(
                user, ConstantsSettings.TECHNOLOGY_illumina
            ),
        )
        self.assertEqual(
        ...
        parameter.parameter = (
            SoftwareNames.SOFTWARE_TRIMMOMATIC_addapter_vect_available[1]
        )
        parameter.save()
        self.assertEqual(
            "ILLUMINACLIP:/usr/local/software/insaflu/trimmomatic/adapters/All-adapters.fa:3:30:10:6:true CROP:42334 "
            + "SLIDINGWINDOW:5:20 LEADING:3 TRAILING:3 MINLEN:35 TOPHRED33",
            default_software.get_trimmomatic_parameters(user),
        )
        ...
        try:
            software = Software.objects.get(
                name=SoftwareNames.SOFTWARE_NanoFilt_name,
                owner=user,
                type_of_use=Software.TYPE_OF_USE_global,
                technology__name=ConstantsSettings.TECHNOLOGY_illumina,
            )
        ...
        parameter.save()
        self.assertEqual(
            "-q 20 -l 50 --headcrop 70 --tailcrop 70",
            default_software.get_nanofilt_parameters(user),
        )
        ...
        parameter.save()
        self.assertEqual(
            "-q 20 -l 50 --headcrop 70 --tailcrop 70 --maxlength 20",
            default_software.get_nanofilt_parameters(user),
        )
        self.assertEqual(
            "-q 20 -l 50 --headcrop 70 --tailcrop 70 --maxlength 20",
        ...
        parameter.save()
        self.assertEqual(
            "-m 20", default_software.get_medaka_parameters_consensus(user)
        )
        self.assertEqual(
            "-m 20",
            default_software.get_parameters(
        ...
        try:
            software = Software.objects.get(
                name=SoftwareNames.INSAFLU_PARAMETER_LIMIT_COVERAGE_ONT_name,
                owner=user,
                type_of_use=Software.TYPE_OF_USE_global,
                technology__name=ConstantsSettings.TECHNOLOGY_illumina,
            )
        ...
        self.assertEqual(
            "Threshold:30", default_software.get_limit_coverage_ONT_parameters(user)
        )
        parameter = parameters[0]
        parameter.parameter = "20"
        parameter.save()
        self.assertEqual(
            "Threshold:20", default_software.get_limit_coverage_ONT_parameters(user)
        )
        self.assertEqual(
            "Threshold:20",
            default_software.get_parameters(
        ...
        try:
            software = Software.objects.get(
                name=SoftwareNames.INSAFLU_PARAMETER_VCF_FREQ_ONT_name,
                owner=user,
                type_of_use=Software.TYPE_OF_USE_global,
                technology__name=ConstantsSettings.TECHNOLOGY_illumina,
            )
        ...
        self.assertEqual("Threshold", parameters[0].name)
        self.assertEqual(
            "Threshold:0.80", default_software.get_vcf_freq_ONT_parameters(user)
        )
        parameter = parameters[0]
        parameter.parameter = "0.1"
        parameter.save()
        self.assertEqual(
            "Threshold:0.1", default_software.get_vcf_freq_ONT_parameters(user)
        )
        self.assertEqual(
            "Threshold:0.1",
            default_software.get_parameters(
        ...
        try:
            software = Software.objects.get(
                name=SoftwareNames.SOFTWARE_ABRICATE_name,
                owner=user,
                type_of_use=Software.TYPE_OF_USE_global,
                technology__name=ConstantsSettings.TECHNOLOGY_illumina,
            )
    ...
    def test_default_project_sample_software_1(self):
        ...
        self.assertEqual(
            "--mapqual 20 --mincov 10 --minfrac 0.51",
            default_software.get_snippy_parameters_all_possibilities(
                user, project_sample
            ),
        )
        self.assertEqual(
            "Threshold:70",
            default_software.get_mask_consensus_parameters_all_possibilities(
                user, project_sample, ConstantsSettings.TECHNOLOGY_illumina
            ),
        )
        self.assertEqual(
            "Threshold:70",
            default_software.get_mask_consensus_parameters_all_possibilities(
                user, project_sample, ConstantsSettings.TECHNOLOGY_minion
            ),
        )

        try:
            software = Software.objects.get(
                name=SoftwareNames.INSAFLU_PARAMETER_MASK_CONSENSUS_name,
                owner=user,
        ...
        self.assertTrue(
            default_software.can_change_values_for_this_software(
                software, project, None, None
            )
        )
        ...
        self.assertEqual(
            "-q 20 -l 50 --headcrop 70 --tailcrop 70",
            default_software.get_nanofilt_parameters(
                user, Software.TYPE_OF_USE_sample, sample
            ),
        )
        self.assertEqual(
            "-q 20 -l 50 --headcrop 70 --tailcrop 70",
            default_software.get_nanofilt_parameters_all_possibilities(user, sample),
        )
        self.assertEqual(
            "20",
            default_software.get_nanofilt_single_parameter(
                sample, DefaultParameters.NANOfilt_quality_read
            ),
        )

        self.assertFalse(
            default_software.is_nanofilt_single_parameter_default_for_project(
                sample, DefaultParameters.NANOfilt_quality_read
            )
        )
        self.assertTrue(
            default_software.can_change_values_for_this_software(
                software, None, None, sample
            )
        )
        ...
        self.assertFalse(
            default_software.is_medaka_single_parameter_default_for_project(
                project, DefaultParameters.MEDAKA_model
            )
        )
        ...
        self.assertEqual(
            "Threshold:30",
            default_software.get_limit_coverage_ONT_parameters(
                user, Software.TYPE_OF_USE_project, project, None
            ),
        )
        parameter = parameters[0]
        ...
        self.assertFalse(
            default_software.is_limit_coverage_ONT_single_parameter_default_for_project(
                project, DefaultParameters.MASK_CONSENSUS_threshold
            )
        )
        self.assertEqual(
            "20",
            default_software.get_limit_coverage_ONT_single_parameter(
        ...
        self.assertEqual(
            "Threshold:0.80",
            default_software.get_freq_vcf_ONT_parameters(
                user, Software.TYPE_OF_USE_project, project, None
            ),
        )
        self.assertTrue(
            default_software.is_freq_vcf_ONT_single_parameter_default_for_project(
                project, DefaultParameters.MASK_CONSENSUS_threshold
            )
        )
        ...
        self.assertEqual(
            "0.12",
            default_software.get_freq_vcf_ONT_single_parameter(
                project_sample, DefaultParameters.MASK_CONSENSUS_threshold
            ),
        )
        self.assertEqual(
    ...
    def test_default_software_project(self):
        ...
        self.assertEqual(
            "--mapqual 20 --mincov 10 --minfrac 0.51",
            default_software.get_snippy_parameters(
                user, Software.TYPE_OF_USE_project, project, None
            ),
        )
        self.assertEqual(
            "--min-mapping-quality 20 --min-base-quality 20 --min-coverage 100 --min-alternate-count 10 "
            + "--min-alternate-fraction 0.01 --ploidy 2 -V",
            default_software.get_freebayes_parameters(
                user, Software.TYPE_OF_USE_project, project, None
            ),
        )
        self.assertEqual(
            "--min-mapping-quality 20 --min-base-quality 20 --min-coverage 100 --min-alternate-count 10 "
            + "--min-alternate-fraction 0.01 --ploidy 2 -V",
            SoftwareNames.SOFTWARE_FREEBAYES_PARAMETERS,
        )
        self.assertEqual(
            "Threshold:70",
            default_software.get_mask_consensus_parameters_for_project(
                user, project, ConstantsSettings.TECHNOLOGY_illumina
            ),
        )
        self.assertEqual(
            "Threshold:70",
            default_software.get_mask_consensus_parameters_for_project(
                user, project, ConstantsSettings.TECHNOLOGY_minion
            ),
        )
        ...
        default_software.set_default_software(
            software[0], user, Software.TYPE_OF_USE_project, project, None, None
        )
        parameters = Parameter.objects.filter(software=software[0], project=project)
        self.assertEqual("0.51", parameters[2].parameter)
        ...
        self.assertEqual(
            True,
            default_software.is_mask_consensus_single_parameter_default(
                project_sample,
        ...
        self.assertFalse(
            default_software.is_snippy_single_parameter_default(
        ...
        self.assertNotEqual("0.222", parameters[2].parameter)
        self.assertTrue(
            default_software.is_change_values_for_software(
                software[0].name,
                ConstantsSettings.TECHNOLOGY_illumina,
        ...
        self.assertEqual(
            "80",
            default_software.get_mask_consensus_single_parameter_for_project(
                project,
                DefaultParameters.MASK_CONSENSUS_threshold,
                ConstantsSettings.TECHNOLOGY_illumina,
            ),
        ...
        try:
            software = Software.objects.get(
                name=SoftwareNames.SOFTWARE_MASK_CONSENSUS_BY_SITE_name,
                owner=user,
                type_of_use=Software.TYPE_OF_USE_project,
        ...
        self.assertEqual(
            "SLIDINGWINDOW:5:20 LEADING:3 TRAILING:3 MINLEN:35 TOPHRED33",
            default_software.get_trimmomatic_parameters(
                user, Software.TYPE_OF_USE_sample, sample
            ),
        )
        parameter = parameters[1]
        ...
        parameter.save()
        self.assertEqual(
            "HEADCROP:20 SLIDINGWINDOW:5:20 LEADING:3 TRAILING:3 MINLEN:35 TOPHRED33",
            default_software.get_trimmomatic_parameters(
                user, Software.TYPE_OF_USE_sample, sample
            ),
        )
        ...
        self.assertFalse(
            default_software.is_trimmomatic_single_parameter_default_for_project(
                sample, "HEADCROP"
            )
        )
        ...
        self.assertTrue(
            default_software.is_abricate_single_parameter_default_for_project(
                sample, "--minid", ConstantsSettings.TECHNOLOGY_illumina
            )
        )
    ...
    def test_default_software_project_sample(self):
        ...
        self.assertEqual(
            "--mapqual 20 --mincov 10 --minfrac 0.51",
            default_software.get_snippy_parameters(
                user, Software.TYPE_OF_USE_project_sample, None, project_sample
            ),
        )
        self.assertEqual(
            "--min-mapping-quality 20 --min-base-quality 20 --min-coverage 100 --min-alternate-count 10 "
        ...
        parameters = Parameter.objects.filter(
            software=software[0], project_sample=project_sample
        )
        self.assertTrue(3, len(parameters))
        ...
        default_software.set_default_software(
            software[0], user, Software.TYPE_OF_USE_project, None, project_sample, None
        )
        self.assertFalse(
            default_software.is_change_values_for_software(
                software[0].name,
