...
class Test(TestCase):
    ...
    def test_result(self):
        ...
        self.assertEqual(
            len(result_2.outputs.list_output), len(result.outputs.list_output)
        )
        self.assertEqual(
            result_2.outputs.list_output[0].file_name,
            result.outputs.list_output[0].file_name,
        )
        self.assertEqual(
            len(result_2.softwares.list_software), len(result.softwares.list_software)
        )
        self.assertEqual(
            result_2.softwares.list_software[0].name,
            result.softwares.list_software[0].name,
        )
    ...
    def test_result_software(self):
        ...
        result.add_output(Output("name", "path"))
        result.outputs.list_output[0].set_software(
            SoftwareDesc("name44_s", "4444", "sdfd")
        )
        ...
        self.assertEqual(
            result_2.outputs.list_output[0].software.name,
            result.outputs.list_output[0].software.name,
        )
        self.assertEqual(
            result_2.outputs.list_output[0].software.parameters,
            result.outputs.list_output[0].software.parameters,
        )
        self.assertEqual(
            len(result_2.softwares.list_software), len(result.softwares.list_software)
        )
    ...
    def test_ResultSoftware(self):
        ...
        result.add_software(
            SoftwareDesc(
                ...
                SoftwareNames.SOFTWARE_SPAdes_VERSION,
                SoftwareNames.SOFTWARE_SPAdes_PARAMETERS,
            )
        )
        result.add_software(
            SoftwareDesc(
                SoftwareNames.SOFTWARE_TRIMMOMATIC_name,
                SoftwareNames.SOFTWARE_TRIMMOMATIC_VERSION,
                SoftwareNames.SOFTWARE_TRIMMOMATIC_PARAMETERS,
            )
        )
        ...
        self.assertEqual(
            "Trimmomatic-0.39; (SLIDINGWINDOW:5:20 LEADING:3 TRAILING:3 MINLEN:35 TOPHRED33)",
            result.get_software(SoftwareNames.SOFTWARE_TRIMMOMATIC_name),
        )
        self.assertEqual(
            "SPAdes-4.0.0; (--only-assembler)",
            result.get_software(SoftwareNames.SOFTWARE_SPAdes_name),
        )
        ...
        self.assertFalse(
            result.is_software_present(SoftwareNames.SOFTWARE_ABRICATE_name)
        )
    ...
    def test_Coverage(self):
        ...
        try:
            self.assertTrue(
                "19", coverage_2.get_coverage("4", Coverage.COVERAGE_MORE_9)
            )
    ...
    def test_elements_genes(self):
        ...
        self.assertTrue(
            geneticElement.add_gene("element_name", 100, Gene("name2", 35, 55, -1))
        )
        self.assertTrue(
            geneticElement.add_gene("element_name2", 200, Gene("name", 12, 45, 1))
        )
        ...
        self.assertEquals(
            "element,element_name,element_name2",
            ",".join(geneticElement_2.get_sorted_elements()),
        )
    ...
    def test_elements_genes_2(self):
        ...
        self.assertTrue(
            geneticElement.add_gene(
                "element_name2", 200, Gene("name", 12, 45, 1, vect_feature_location)
            )
        )
        geneticElement.add_gene("element_name2", 200, Gene("name2", 212, 245, 1))
        geneticElement.add_gene("element_name2", 200, Gene("name3", 412, 445, -1))
        ...
        self.assertEqual(
            FeatureLocationSimple(265, 13468, 1), gene.get_feature_locations()[0]
        )
        self.assertEqual(
            FeatureLocationSimple(13467, 21555, 1), gene.get_feature_locations()[1]
        )
        ...
        self.assertTrue(
            geneticElement_3.add_gene("element_name", 100, Gene("name", 12, 45, 1))
        )
        self.assertFalse(
            geneticElement_3.add_gene("element_name", 100, Gene("name", 12, 45, 1))
        )
        self.assertTrue(
            geneticElement_3.add_gene("element_name", 100, Gene("name2", 35, 55, -1))
        )
        self.assertTrue(
            geneticElement_3.add_gene("element_name2", 200, Gene("name", 12, 45, 1))
        )
    ...
    def test_process_results(self):
        ...
        process_results.add_single_result(
            SingleResult(SingleResult.ERROR, "xprto errroer")
        )
    ...
    def test_masking_consensus(self):
        ...
        self.assertEqual(
            "Element:xpto -> No mask",
            masking_consensus.get_message_to_show_in_web_site("xpto"),
        )
        ...
        self.assertEqual(
            "mask_sites: 2,5  mask_from_beginning: 2   mask_from_ends: 2  mask_regions: 1-30,32-50,320-500",
            str(masking_consensus),
        )
        ...
        self.assertEqual(
            "mask_sites: 2,5  mask_from_beginning: 2   mask_from_ends: 2  mask_regions: 1-30,32-50,320-500",
            str(masking_consensus_2),
        )
        self.assertEqual(
            "Element:xpto\n\nMask sites:2,5  Mask regions:1-30,32-50,320-500\nMask from beginning:2  Mask from ends:2",
            masking_consensus_2.get_message_to_show_in_web_site("xpto"),
        )
        self.assertEqual(
            "Mask sites,Mask from beginning,Mask from end,Mask regions",
            masking_consensus.get_header(","),
        )
