...
class CollectExtraData(object):
    ...
    HEADER_SAMPLE_OUT_CSV = (
        HEADER_SAMPLE_OUT_ID
        + ",fastq1,fastq2,data set,vaccine status,week,onset date,collection date,lab reception date,latitude,longitude,classification,putative mixed-infection"
    )
    HEADER_SAMPLE_OUT_CSV_statistics = (
        HEADER_SAMPLE_OUT_ID + ",fastq1,fastq2,sample date created"
    )
    HEADER_SAMPLE_OUT_CSV_simple = (
        HEADER_SAMPLE_OUT_ID
        + ",data set,vaccine status,week,onset date,collection date,lab reception date,latitude,longitude,classification,putative mixed-infection"
    )
    ...
    def collect_update_pangolin_lineage(self, project, user):
        ...
        self.calculate_global_files(
            Project.PROJECT_FILE_NAME_SAMPLE_RESULT_all_consensus, project, user
        )
    ...
    def __collect_update_pangolin_lineage(self, project, user, b_mark_sge_success=True):
        ...
        try:
            ...
            file_consensus = project.get_global_file_by_project(
                TypePath.MEDIA_ROOT,
                Project.PROJECT_FILE_NAME_SAMPLE_RESULT_all_consensus,
            )
            ## test if is necesserary to run pangolin lineage
            if (
                os.path.exists(file_pangolin_output)
                or self.software.get_species_tag(project.reference)
                == Reference.SPECIES_SARS_COV_2
            ):
                ## process pangolin
                self.software_pangolin.run_pangolin(
                    file_consensus, file_pangolin_output
                )
                try:
                    ...
                    for soft_name in dt_result_version:
                        result_all.add_software(
                            SoftwareDesc(
                                soft_name, dt_result_version.get(soft_name, ""), ""
                            )
                        )
                    ## Add Analysis Mode that was ran
                    result_all.add_software(
                        SoftwareDesc(
                            SoftwareNames.SOFTWARE_Pangolin_analysis_mode,
                    ...
                    for project_sample in ProjectSample.objects.filter(project=project):
                        if (
                            ...
                            in pangolin_data["Sequence"].values
                        ):
                            project_sample.classification = pangolin_data.loc[
                                pangolin_data["Sequence"] == project_sample.sample.name,
                                "lineage",
                            ].values[0]
                        project_sample.save()
                    ...
                    self.logger_production.error(
                        "ProjectID: {}  Fail to run pangolin".format(project.id)
                    )
        ...
        try:
            ...
            for projectsample in ProjectSample.objects.filter(project=project):
                self.software.run_classification(
                    projectsample=projectsample, owner=user
                )
    ...
    def __collect_mutation_report(
        ...
        try:
            ...
            if species_tag == Reference.SPECIES_SARS_COV_2:
                ...
                if geneticElement == None:
                    ## finished with error
                    self.logger_debug.info(
                        "Aln2pheno Gave an error, geneticElement is None"
                    )
                    process_SGE.set_process_controler(
                        user,
                        process_controler.get_name_project(project),
    ...
    def __collect_project_flumut_report(self, project: Project):
        ...
        file_flumut_markers_report = project.get_global_file_by_project(
            TypePath.MEDIA_ROOT, Project.PROJECT_FILE_NAME_Flumut_markers_report
        )
        file_flumut_litterature_report = project.get_global_file_by_project(
            TypePath.MEDIA_ROOT, Project.PROJECT_FILE_NAME_Flumut_litterature_report
        )
        ...
        self.software.run_flumut(
            file_alignments,
            file_flumut_markers_report,
            file_flumut_mutation_report,
    ...
    def __collect_aln2pheno(
        ...
        file_aln2pheno_report_COG_UK = project.get_global_file_by_project(
            TypePath.MEDIA_ROOT, Project.PROJECT_FILE_NAME_Aln2pheno_report_COG_UK
        )
        file_aln2pheno_flagged_COG_UK = project.get_global_file_by_project(
            TypePath.MEDIA_ROOT, Project.PROJECT_FILE_NAME_Aln2pheno_flagged_COG_UK
        )
        ...
        self.software.run_aln2pheno(
            reference="{}_{}_{}".format(
                project.reference.name, sequence_name, GENE_NAME
            ),
            gene=GENE_NAME,
            sequences=file_alignments,
            report=tmp_aln2pheno,
            flagged=file_aln2pheno_flagged_COG_UK,
            db="DB_SARS_CoV_2_Spike_COG_UK_Antigenic_2022_10_20.tsv",
        )
        ...
        report_data = report_data[
            [cols[0]] + [cols[len(cols) - 1]] + cols[1 : (len(cols) - 1)]
        ]

        report_data.to_csv(
            file_aln2pheno_report_COG_UK, sep=Constants.SEPARATOR_TAB, index=False
        )
        ...
        file_aln2pheno_report_pokay = project.get_global_file_by_project(
            TypePath.MEDIA_ROOT, Project.PROJECT_FILE_NAME_Aln2pheno_report_pokay
        )
        file_aln2pheno_flagged_pokay = project.get_global_file_by_project(
            TypePath.MEDIA_ROOT, Project.PROJECT_FILE_NAME_Aln2pheno_flagged_pokay
        )
        ...
        self.software.run_aln2pheno(
            ...
            sequences=file_alignments,
            report=tmp_aln2pheno,
            flagged=file_aln2pheno_flagged_pokay,
            db="DB_SARS_CoV_2_Spike_Pokay_2022_07_28.tsv",
        )
        ...
        report_data.to_csv(
            file_aln2pheno_report_pokay, sep=Constants.SEPARATOR_TAB, index=False
        )
        ...
        file_aln2pheno_report_carabelli = project.get_global_file_by_project(
            TypePath.MEDIA_ROOT, Project.PROJECT_FILE_NAME_Aln2pheno_report_carabelli
        )
        file_aln2pheno_flagged_carabelli = project.get_global_file_by_project(
            TypePath.MEDIA_ROOT, Project.PROJECT_FILE_NAME_Aln2pheno_flagged_carabelli
        )
        ...
        self.software.run_aln2pheno(
            ...
            sequences=file_alignments,
            report=tmp_aln2pheno,
            flagged=file_aln2pheno_flagged_carabelli,
            db="DB_SARS_CoV_2_Spike_EpitopeResidues_Carabelli_2023_NatRevMic_Fig1.tsv",
        )
        ...
        report_data.to_csv(
            file_aln2pheno_report_carabelli, sep=Constants.SEPARATOR_TAB, index=False
        )
    ...
    def __collect_update_extra_metadata_for_project(self, project, user):
        ...
        try:
            ...
            self.calculate_global_files(
                Project.PROJECT_FILE_NAME_SAMPLE_RESULT_TSV, project, user
            )
            self.calculate_global_files(
                Project.PROJECT_FILE_NAME_SAMPLE_RESULT_SETTINGS_CSV, project, user
            )
            self.calculate_global_files(
                Project.PROJECT_FILE_NAME_SAMPLE_RESULT_CSV_simple, project, user
            )
            self.calculate_global_files(
                Project.PROJECT_FILE_NAME_SAMPLE_RESULT_SETTINGS_TSV, project, user
            )
            ## IMPORTANT -> this need to be after of Project.PROJECT_FILE_NAME_SAMPLE_RESULT_CSV
            self.calculate_global_files(
                Project.PROJECT_FILE_NAME_SAMPLE_RESULT_json, project, user
            )

            ## (re)calculate the phenotype table, as it uses metadata
            self.__collect_mutation_report(project, user, False)
        ...
        ## seal the tag
        meta_project = manage_database.get_project_metakey_last(
            project,
    ...
    def __collect_extra_data_for_project(self, project, user):
        ...
        try:
            ...
            self.logger_production.info(
                "COLLECT_EXTRA_FILES: max sample label size Step {}  diff_time:{}".format(
            ...
            self.calculate_global_files(
                Project.PROJECT_FILE_NAME_SAMPLE_mask_all_consensus, project, user
            )
            self.logger_production.info(
                "COLLECT_EXTRA_FILES: masks all consensus Step {}  diff_time:{}".format(
                    count, time.time() - start
                )
            )
            self.logger_debug.info(
            ...
            self.logger_production.info(
                "COLLECT_EXTRA_FILES: collect all consensus Step {}  diff_time:{}".format(
                    count, time.time() - start
                )
            )
            self.logger_debug.info(
            ...
            self.logger_production.info(
                "COLLECT_EXTRA_FILES: calculate the classification Step {}  diff_time:{}".format(
                    count, time.time() - start
                )
            )
            self.logger_debug.info(
            ...
            self.__collect_update_pangolin_lineage(project, user, False)
            self.logger_production.info(
                "COLLECT_EXTRA_FILES: calculate the lineage Step {}  diff_time:{}".format(
                    count, time.time() - start
                )
            )
            self.logger_debug.info(
            ...
            self.calculate_global_files(
                Project.PROJECT_FILE_NAME_COVERAGE, project, user
            )
            self.logger_production.info(
                "COLLECT_EXTRA_FILES: calculate coverage Step {}  diff_time:{}".format(
                    count, time.time() - start
                )
            )
            self.logger_debug.info(
            ...
            self.calculate_global_files(
                Project.PROJECT_FILE_NAME_TAB_VARIATIONS_SNIPPY, project, user
            )
            self.logger_production.info(
                "COLLECT_EXTRA_FILES: snippy variants Step {}  diff_time:{}".format(
                    count, time.time() - start
                )
            )
            self.logger_debug.info(
            ...
            ## remove del or ins
            self.calculate_global_files(
                Project.PROJECT_FILE_NAME_TAB_VARIATIONS_FREEBAYES, project, user
            )
            self.logger_production.info(
                "COLLECT_EXTRA_FILES: freebayes variants Step {}  diff_time:{}".format(
                    count, time.time() - start
                )
            )
            self.logger_debug.info(
            ...
            self.calculate_global_files(
                Project.PROJECT_FILE_NAME_TAB_VARIATIONS_FREEBAYES_with_snps_indels,
                project,
                user,
            )
            self.logger_production.info(
                "COLLECT_EXTRA_FILES: freebayes variants 2 Step {}  diff_time:{}".format(
                    count, time.time() - start
                )
            )
            self.logger_debug.info(
            ...
            self.logger_production.info(
                "COLLECT_EXTRA_FILES: sample table Step {}  diff_time:{}".format(
                    count, time.time() - start
                )
            )
            self.logger_debug.info(
            ...
            self.logger_production.info(
                "COLLECT_EXTRA_FILES: sample json Step {}  diff_time:{}".format(
                    count, time.time() - start
                )
            )
            self.logger_debug.info(
            ...
            self.logger_production.info(
                "COLLECT_EXTRA_FILES: count variations Step {}  diff_time:{}".format(
                    count, time.time() - start
                )
            )
            self.logger_debug.info(
            ...
            self.logger_production.info(
                "COLLECT_EXTRA_FILES: trees and alignments Step {}  diff_time:{}".format(
                    count, time.time() - start
                )
            )
            self.logger_debug.info(
            ...
            self.__collect_mutation_report(project, user, False)
            self.logger_production.info(
                "COLLECT_EXTRA_FILES: aln2pheno Step {}  diff_time:{}".format(
                    count, time.time() - start
                )
            )
            self.logger_debug.info(
            ...
            self.logger_production.info(
                "COLLECT_EXTRA_FILES: project list Step {}  diff_time:{}".format(
                    count, time.time() - start
                )
            )
            self.logger_debug.info(
            ...
            self.logger_production.info(
                "COLLECT_EXTRA_FILES: sample list Step {}  diff_time:{}".format(
                    count, time.time() - start
                )
            )
            self.logger_debug.info(
            ...
            self.zip_several_files(project)
            self.logger_production.info(
                "COLLECT_EXTRA_FILES: zip files Step {}  diff_time:{}".format(
                    count, time.time() - start
                )
            )
            self.logger_debug.info(
    ...
    def create_graph_minor_variants(self, project, user):
        ...
        for project_sample in project.project_samples.all():
            ...
            vect_data_sample.append(
                [
                    project_sample.sample.name,
                    count_hits.get_total(),
                    count_hits.get_hits_less_50(),
                    count_hits.get_hits_50_90(),
                ]
            )

        ### there's no data to create the graph
        if len(vect_data_sample) == 0:
            meta_data = manageDatabase.set_project_metakey(
                project,
                user,
    ...
    def create_graph_plotly(self, vect_data_sample):
        ...
        trace2 = go.Bar(
            ...
            marker=dict(
                ...
                line=dict(color="rgba(50, 96, 171, 1.0)", width=3),
            ),
        )
    ...
    def calculate_global_files(self, type_file, project, user):
        ...
        if type_file == Project.PROJECT_FILE_NAME_COVERAGE:
            ...
            out_file = self.collect_sample_table(
                project, Constants.SEPARATOR_COMMA, CollectExtraData.SAMPLE_LIST_list
            )
            out_file_file_system = project.get_global_file_by_project(
                TypePath.MEDIA_ROOT, type_file
            )
            ...
            out_file = self.collect_sample_table(
                ...
                CollectExtraData.SAMPLE_LIST_list_settings,
            )
            out_file_file_system = project.get_global_file_by_project(
                TypePath.MEDIA_ROOT, type_file
            )
            ...
            out_file = self.collect_sample_table(
                project, Constants.SEPARATOR_COMMA, CollectExtraData.SAMPLE_LIST_simple
            )
            out_file_file_system = project.get_global_file_by_project(
                TypePath.MEDIA_ROOT, type_file
            )
            ...
            ## samples tsv
            out_file = self.collect_sample_table(
                project, Constants.SEPARATOR_TAB, CollectExtraData.SAMPLE_LIST_list
            )
            out_file_file_system = project.get_global_file_by_project(
                TypePath.MEDIA_ROOT, type_file
            )
    ...
    def create_json_file_from_sample_csv(self, project):
        ...
        file_name_root_sample = project.get_global_file_by_project(
            TypePath.MEDIA_ROOT, Project.PROJECT_FILE_NAME_SAMPLE_RESULT_CSV_simple
        )
                ...
                if len(dt_result) == len(all_data):
                    ...
                    self.logger_production.error(
                        "ProjectID: {}  different number of lines processing Sample {} -> JSON {}".format(
                            project.id, len(dt_result), len(all_data)
                        )
                    )
                    self.logger_debug.error(
                        "ProjectID: {}  different number of lines processing Sample {} -> JSON {}".format(
    ...
    def create_coverage_file(self, project, user):
        ...
        vect_ratios = [
            ...
            "% of size covered by at least X-fold",
        ]
        ...
        with open(out_file, "w", newline="") as output_file_handle_csv:
            ...
            for _, ratio in enumerate(vect_ratios):
                vect_out.append("")
                if _ == len(vect_ratios) - 1:
                    vect_out.append("X-Fold")
                vect_out.extend(vect_reference)
            csv_writer.writerow(vect_out)
            ...
            for project_sample in project.project_samples.all():
                ...
                for element_name in vect_reference:
                    vect_out.append(
                        coverage.get_coverage(element_name, Coverage.COVERAGE_ALL)
                    )

                vect_out.append("")
                for element_name in vect_reference:
                    vect_out.append(
                        coverage.get_coverage(element_name, Coverage.COVERAGE_MORE_0)
                    )
    ...
    def collect_variations_snippy(self, project: Project):
        ...
        out_file = self.utils.get_temp_file(
            "variations_snippy", FileExtensions.FILE_TSV
        )
    ...
    def mask_all_consensus_files(self, project):
        ...
        if not meta_value is None:
            masking_consensus_original_project = decode_masking_consensus.decode_result(
        ...
        for project_sample in project.project_samples.all():
            if (
                not project_sample.get_is_ready_to_proccess()
                or project_sample.is_deleted
            ):
                continue
            ...
            if not meta_value is None:
                masking_consensus_original_project_sample = (
                    decode_masking_consensus.decode_result(meta_value.description)
                )
            ...
            if (
                not masking_consensus_original_project is None
                and masking_consensus_original_project.has_masking_data()
            ):
                self.software.mask_sequence_by_sites(
                    project.reference.get_reference_fasta(TypePath.MEDIA_ROOT),
                    project_sample.get_consensus_file(TypePath.MEDIA_ROOT),
                    masking_consensus_original_project,
                )
    ...
    def merge_all_consensus_files(self, project):
        ...
        for project_sample in project.project_samples.all():
            ...
            if not default_software.include_consensus(project_sample):
                continue
    ...
    def zip_several_files(self, project):
        ...
        if os.path.exists(
            ...
            self.utils.link_file(
                ...
                os.path.join(
                    temp_dir, Project.PROJECT_FILE_NAME_TAB_VARIATIONS_FREEBAYES
                ),
            )
        ...
        if os.path.exists(
            ...
            self.utils.link_file(
                ...
                os.path.join(
                    temp_dir, Project.PROJECT_FILE_NAME_SAMPLE_RESULT_SETTINGS_CSV
                ),
            )
        if os.path.exists(
            project.get_global_file_by_project(
            ...
            self.utils.link_file(
                ...
                os.path.join(
                    temp_dir, Project.PROJECT_FILE_NAME_SAMPLE_RESULT_SETTINGS_TSV
                ),
            )
        ...
        if os.path.exists(
            ...
            self.utils.link_file(
                ...
                os.path.join(
                    temp_dir, Project.PROJECT_FILE_NAME_SAMPLE_RESULT_all_consensus
                ),
            )
    ...
    def collect_variations_freebayes(self, project, vect_type_remove):
        ...
        """
        out_file = self.utils.get_temp_file(
            "variations_freebayes", FileExtensions.FILE_TSV
        )
        parse_out_files = ParseOutFiles()
        n_count = 0
        vect_type_out = []
        ...
        with open(out_file, "w", newline="") as handle_out:
            ...
            for project_sample in project.project_samples.all():
                ...
                if not os.path.exists(tab_file_to_process):
                    continue
                parse_out_files.parse_tab_files(
                    project_sample.sample.name,
                    tab_file_to_process,
                    csv_writer,
                    vect_type_out,
                    vect_type_remove,
                    Project.PERCENTAGE_validated_minor_variants,
                    True if n_count == 0 else False,
                )
                n_count += 1
        if n_count == 0:
    ...
    def collect_variations_freebayes_only_one_file(
        self, file_in, file_out, vect_type_remove
    ):
        """
        collect freebayes variations
        """
        ...
        with open(file_out, "w", newline="") as handle_out:
            ...
            count_print = parse_out_files.parse_tab_files(
                None,
                file_in,
                csv_writer,
                vect_type_out,
    ...
    def collect_sample_table(self, project, column_separator, type_list):
        ...
        parse_pangolin = ParsePangolinResult(
            project.get_global_file_by_project(
                TypePath.MEDIA_ROOT, Project.PROJECT_FILE_NAME_Pangolin_lineage
            )
        )
        ...
        with open(out_file, "w", newline="") as handle_out:
            ...
            elif type_list == CollectExtraData.SAMPLE_LIST_simple:
                vect_out_header = CollectExtraData.HEADER_SAMPLE_OUT_CSV_simple.split(
                    ","
                )
            ## settings
            else:
                vect_out_header = (
                    CollectExtraData.HEADER_SAMPLE_OUT_CSV_statistics.split(",")
                )
            ...
            if type_list in [
                ...
                if parse_pangolin.has_data():
                    vect_out_header.extend(["Lineage Pangolin", "Scorpio Pangolin"])
            if type_list == CollectExtraData.SAMPLE_LIST_list_settings:
                vect_out_header.extend(
                    [
                        "Technology",
                        "Sample Down sized",
                        "Available Reads for Mapping",
                        "Mapped Reads",
                        "Mapped Percentage",
                    ]
                )
            ...
            if type_list == CollectExtraData.SAMPLE_LIST_list_settings:
                ...
                if not b_trimmomatic_stats and not b_nanostat_sats:
                    ...
                    if b_nanostat_sats:
                        vect_out.extend(
                            ["NanoStat stats."]
                            * len(SoftwareNames.SOFTWARE_NANOSTAT_vect_info_to_collect)
                        )
                    vect_out.extend(
                        [""]
                        * (
                            len(vect_tags_out_sample)
            ...
            if type_list == CollectExtraData.SAMPLE_LIST_simple:
                csv_writer.writerow(
                    [project.reference.name] + [""] * (len(vect_out_header) - 1)
                )
            ...
            for project_sample in project.project_samples.all():
                ...
                if type_list in [
                    ...
                    if type_list == CollectExtraData.SAMPLE_LIST_list:
                        ...
                        ### fastq2
                        vect_out.append(
                            project_sample.sample.file_name_2
                            if project_sample.sample.file_name_2 != None
                            and len(project_sample.sample.file_name_2) > 0
                            else ""
                        )

                    ### dataset
                    vect_out.append(
                        project_sample.sample.data_set.name
                        if project_sample.sample.data_set != None
                        else ""
                    )
                    ### vaccine status
                    vect_out.append(
                        project_sample.sample.vaccine_status.name
                        if project_sample.sample.vaccine_status != None
                        else ""
                    )
                    ### week
                    vect_out.append(
                        str(project_sample.sample.week)
                        if project_sample.sample.week != None
                        else ""
                    )
                    ### onset date
                    vect_out.append(
                        project_sample.sample.date_of_onset.strftime(
                            settings.DATE_FORMAT_FOR_SHOW
                        )
                        if project_sample.sample.date_of_onset != None
                        else ""
                    )
                    ### collection date
                    vect_out.append(
                        project_sample.sample.date_of_collection.strftime(
                            settings.DATE_FORMAT_FOR_SHOW
                        )
                        if project_sample.sample.date_of_collection != None
                        else ""
                    )
                    ### lab reception date
                    vect_out.append(
                        project_sample.sample.date_of_receipt_lab.strftime(
                            settings.DATE_FORMAT_FOR_SHOW
                        )
                        if project_sample.sample.date_of_receipt_lab != None
                        else ""
                    )
                    ### latitude
                    vect_out.append(
                        str(project_sample.sample.geo_local.coords[0])
                        if project_sample.sample.geo_local != None
                        else ""
                    )
                    ### longitude
                    vect_out.append(
                        str(project_sample.sample.geo_local.coords[1])
                        if project_sample.sample.geo_local != None
                        else ""
                    )
                    ### type_subtype
                    if parse_pangolin.has_data():
                        vect_out.append(
                            ...
                            if project_sample.sample.type_subtype != None
                            else ""
                        )
                    else:
                        vect_out.append(
                            project_sample.classification
                            if project_sample.classification != None
                            else ""
                        )
                    ### mixedinfection
                    vect_out.append(
                        project_sample.mixed_infections.tag.name
                        if project_sample.mixed_infections != None
                        else ""
                    )
                    ...
                    if parse_pangolin.has_data():
                        vect_out.append(
                            parse_pangolin.get_value(
                                project_sample.seq_name_all_consensus,
                                ParsePangolinResult.KEY_LINEAGE,
                            )
                        )
                        vect_out.append(
                            parse_pangolin.get_value(
                                project_sample.seq_name_all_consensus,
                                ParsePangolinResult.KEY_SCORPIO,
                            )
                        )
                ...
                ### information about the software, CollectExtraData.SAMPLE_LIST_list_settings
                    ...
                    if settings.RUNNING_TEST:
                        vect_out.append("2010-10-23")
                    else:
                        vect_out.append(
                            project_sample.sample.creation_date.strftime(
                                settings.DATETIME_FORMAT_FOR_SHOW
                            )
                        )
                    ...
                    vect_out.append(
                        "True"
                        if manage_database.is_sample_downsized(project_sample.sample)
                        else "False"
                    )
                    ...
                    if project_sample.id in dict_all_results:
                        ...
                        if b_trimmomatic_stats:
                            self._get_info_from_trimmomatic_stats(
                                vect_out, project_sample.id, dict_all_results
                            )
                        ### nanostat stats
                        if b_nanostat_sats:
                            self._get_info_from_nanostats_stats(
                                vect_out, project_sample.id, dict_all_results
                            )

                        ### samples NAME_sample
                        self._get_info_for_software(
                            MetaKeyAndValue.NAME_sample,
                            vect_out,
                            dict_all_results,
                            project_sample.id,
                            vect_tags_out_sample,
                        ...
                        self._get_info_for_software(
                            MetaKeyAndValue.NAME_project_sample,
                            vect_out,
                            dict_all_results,
                            project_sample.id,
                            vect_tags_out_project_sample,
                        ...
                        vect_out.extend(
                            ...
                            * (
                                ...
                                + len(
                                    SoftwareNames.SOFTWARE_NANOSTAT_vect_info_to_collect
                                    if b_nanostat_sats
                                    else 0
                                )
                            )
    ...
    def _get_mapped_stats_info(self, project_sample: ProjectSample):
        ...
        manage_database = ManageDatabase()
        meta_value = manage_database.get_project_sample_metakey_last(
            project_sample,
        ...
        if meta_value is None:
            ...
            manage_database.set_project_sample_metakey(
                project_sample,
                project_sample.sample.owner,
                MetaKeyAndValue.META_KEY_bam_stats,
                MetaKeyAndValue.META_VALUE_Success,
        ...
        dt_data, total_reads = self.software.get_stats_from_sample_reads(
            project_sample.sample
        )
        mapped_reads = result.get_value_by_key(
            MetaKeyAndValue.SAMTOOLS_flagstat_mapped_reads
        )
        if (
            not mapped_reads is None
            and not total_reads is None
            and self.utils.is_integer(mapped_reads)
            and self.utils.is_integer(total_reads)
        ):
            if int(total_reads) > 0:
                return [
                    str(total_reads),
                    mapped_reads,
                    "{:.1f}".format((int(mapped_reads) / float(total_reads)) * 100),
                ]
            return [str(total_reads), str(mapped_reads), "nan"]
        return ["-"] * 3

    def _get_info_for_software(
        self,
        tag_to_search,
        vect_out,
        dict_all_results,
        ...
        if tag_to_search in dict_all_results[project_sample_id]:
            for software_name in vect_tags_info:
                ...
                for result in dict_all_results[project_sample_id][tag_to_search]:
                    software_result = result.get_software(
                        software_name, software_name in vect_not_add_software_name
                    )
                    if len(software_result) > 0:
                        break
                vect_out.append(software_result)
        else:
            vect_out.extend([""] * len(vect_tags_info))

    def _get_info_from_trimmomatic_stats(
        self, vect_out, project_sample_id, dict_all_results
    ):
        """
        return the stats of trimmomatic
        """
        if MetaKeyAndValue.NAME_sample in dict_all_results[project_sample_id]:
            for result in dict_all_results[project_sample_id][
                MetaKeyAndValue.NAME_sample
            ]:
                software = result.get_software_instance(
                    SoftwareNames.SOFTWARE_TRIMMOMATIC_name
                )
                if not software is None and not software.get_vect_key_values() is None:
                    for (
                        stat_names
                    ) in SoftwareNames.SOFTWARE_TRIMMOMATIC_vect_info_to_collect:
                        b_found = False
    ...
    def _get_info_from_nanostats_stats(
        self, vect_out, project_sample_id, dict_all_results
    ):
        """
        return the stats of trimmomatic
        ...
        if MetaKeyAndValue.NAME_sample in dict_all_results[project_sample_id]:
            for result in dict_all_results[project_sample_id][
                ...
                vect_soft = result.get_list_software_instance(
    ...
    def get_tags_for_samples_in_projects(self, project):
        ...
        for project_sample in project.project_samples.all():
            ...
            for tag_name in project_sample.sample.tag_names.all():
                if tag_name.name in dict_tags_out:
                    continue
    ...
    def get_list_of_softwares_used(self, project):
        ...
        meta_value = manage_database.get_project_metakey_last(
            project,
            MetaKeyAndValue.META_KEY_Masking_consensus,
            MetaKeyAndValue.META_VALUE_Success,
        )
        masking_consensus_original_project = None
        if not meta_value is None:
            masking_consensus_original_project = decode_result.decode_result(
                meta_value.description
            )
        ...
        for project_sample in project.project_samples.all():
            ...
            for technology in MetaKeyAndValue.VECT_TECHNOLOGIES_OUT_REPORT:
                if (
                    MetaKeyAndValue.NAME_sample
                    in MetaKeyAndValue.DICT_SOFTWARE_SHOW_IN_RESULTS[technology]
                ):
                    for (
                        keys_for_sample
                    ) in MetaKeyAndValue.DICT_SOFTWARE_SHOW_IN_RESULTS[technology].get(
                        MetaKeyAndValue.NAME_sample
                    ):
                        ...
                        if (
                            ...
                            if (
                                not soft_desc is None
                                and not soft_desc.get_vect_key_values() is None
                                and len(soft_desc.get_vect_key_values()) > 0
                            ):
                                b_trimmomatic_tags = True
                        if (
                            not b_nanostat_sats
                            and technology == ConstantsSettings.TECHNOLOGY_minion
                        ):
                            soft_desc = result.get_software_instance(
                                SoftwareNames.SOFTWARE_NanoStat_name
                            )
                        ...
                        for software_name in result.get_all_software_names():
                            if not software_name in vect_tags_out_sample:
                                vect_tags_out_sample.append(software_name)
            ...
            for technology in MetaKeyAndValue.VECT_TECHNOLOGIES_OUT_REPORT:
                if (
                    ...
                    for (
                        keys_for_proj_sample
                    ) in MetaKeyAndValue.DICT_SOFTWARE_SHOW_IN_RESULTS[technology].get(
                        MetaKeyAndValue.NAME_project_sample
                    ):
                        meta_sample = manage_database.get_project_sample_metakey_last(
                            project_sample,
                            keys_for_proj_sample,
                            MetaKeyAndValue.META_VALUE_Success,
                        )
            ...
            if not meta_value is None:
                masking_consensus_original_project_sample = decode_result.decode_result(
                    meta_value.description
                )
            else:
                masking_consensus_original_project_sample = (
            ...
            if (
                not masking_consensus_original_project_sample is None
                and masking_consensus_original_project_sample.has_masking_data()
            ):
                result_process = Result()
                result_process.add_software(
                    SoftwareDesc(
                ...
                if (
                    not SoftwareNames.SOFTWARE_MASK_CONSENSUS_BY_SITE_name
                    in vect_tags_out_project_sample
                ):
                    vect_tags_out_project_sample.append(
                        SoftwareNames.SOFTWARE_MASK_CONSENSUS_BY_SITE_name
                    )
                    vect_not_add_software_name_output.append(
                        SoftwareNames.SOFTWARE_MASK_CONSENSUS_BY_SITE_name
                    )
            ### END mask consensus by position
            ...
            if not meta_value is None:
                masking_minfrac_project_sample = decode_result.decode_result(
                    meta_value.description
                )
                if masking_minfrac_project_sample.has_masking_data():
                    ...
                    dt_out[MetaKeyAndValue.NAME_project_sample].append(result_process)
                    if (
                        not MetaKeyAndValue.META_KEY_Masking_consensus_by_minfrac_VCF_medaka
                        in vect_tags_out_project_sample
                    ):
                        vect_tags_out_project_sample.append(
                            MetaKeyAndValue.META_KEY_Masking_consensus_by_minfrac_VCF_medaka
            ...
            if not result_pangolin is None:
                dt_out[MetaKeyAndValue.NAME_project_sample].append(result_pangolin)
    ...
    def calculate_count_variations(self, project):
        ...
        destination_file = project.get_global_file_by_project(
        ...
        if len(data_out) > 0:
            ...
            out_file = self.utils.get_temp_file(
                "count_variations", FileExtensions.FILE_TSV
            )
            with open(out_file, "w", newline="") as handle_out:
                csv_writer = csv.writer(
                    handle_out,
            ...
            self.utils.remove_file(destination_file)
        return None
            ...
            for technology in MetaKeyAndValue.VECT_TECHNOLOGIES_OUT_REPORT:
                if (
                    ...
                    for (
                        ...
                        meta_sample = manage_database.get_sample_metakey_last(
                            sample, keys_for_sample, MetaKeyAndValue.META_VALUE_Success
                        )
                        if meta_sample is None:
                            continue
                        result = decode_result.decode_result(meta_sample.description)
                        ...
                        for software_name in result.get_all_software_names():
                            ...
                            if software_name == SoftwareNames.SOFTWARE_ILLUMINA_stat:
                                continue
                            if not software_name in vect_tags_out_sample:
                                vect_tags_out_sample.append(software_name)
    ...
    def collect_sample_list(self, user, b_test=False):
        ...
        lst_samples = Sample.objects.filter(
            owner=user, is_deleted=False, is_ready_for_projects=True, is_obsolete=False
        ...
        if out_file is None:
            self.utils.remove_file(csv_file)
        else:
            self.utils.move_file(out_file, csv_file)
        ...
        if out_file is None:
            self.utils.remove_file(tsv_file)
        else:
            self.utils.move_file(out_file, tsv_file)
    ...
    def _collect_sample_list(self, lst_samples, column_separator, b_test=False):
        ...
        with open(out_file, "w", newline="") as handle_out:
            ...
            for sample in lst_samples:
                ...
                vect_out.append(
                    sample.file_name_2
                    if sample.file_name_2 != None and len(sample.file_name_2) > 0
                    else ""
                )
                ### dataset
                ...
                ### vaccine status
                vect_out.append(
                    sample.vaccine_status.name if sample.vaccine_status != None else ""
                )
                ### week
                vect_out.append(str(sample.week) if sample.week != None else "")
                ### onset date
                vect_out.append(
                    sample.date_of_onset.strftime(settings.DATE_FORMAT_FOR_SHOW)
                    if sample.date_of_onset != None
                    else ""
                )
                ### collection date
                vect_out.append(
                    sample.date_of_collection.strftime(settings.DATE_FORMAT_FOR_SHOW)
                    if sample.date_of_collection != None
                    else ""
                )
                ### lab reception date
                vect_out.append(
                    sample.date_of_receipt_lab.strftime(settings.DATE_FORMAT_FOR_SHOW)
                    if sample.date_of_receipt_lab != None
                    else ""
                )
                ### latitude
                vect_out.append(
                    str(sample.geo_local.coords[0]) if sample.geo_local != None else ""
                )
                ### longitude
                vect_out.append(
                    str(sample.geo_local.coords[1]) if sample.geo_local != None else ""
                )
                ### type_subtype
                vect_out.append(
                    sample.type_subtype if sample.type_subtype != None else ""
                )
                ...
                ### downsized
                vect_out.append(
                    "True" if manage_database.is_sample_downsized(sample) else "False"
                )
                ...
                if sample.id in dict_all_results:
                    ...
                    if b_trimmomatic_stats:
                        self._get_info_from_trimmomatic_stats(
                            vect_out, sample.id, dict_all_results
                        )
                    ### nanostat stats
                    if b_nanostat_sats:
                        self._get_info_from_nanostats_stats(
                ...
                ## creation date
                if b_test:
                    vect_out.append("Date")
                else:
                    vect_out.append(
                        sample.creation_date.strftime(settings.DATETIME_FORMAT_FOR_SHOW)
                    )
                ...
                for project in Project.objects.filter(
                    is_deleted=False,
                    project_samples__sample=sample,
                    project_samples__is_deleted=False,
                ).distinct():
                    if len(sz_out) == 0:
                        sz_out += project.name
                    else:
    ...
    def collect_project_list(self, user, b_test=False):
        ...
        ## get all samples at one step
        lst_projects = Project.objects.filter(owner=user, is_deleted=False).order_by(
            "-creation_date"
        )
    ...
    def _collect_project_list(self, lst_projects, column_separator, b_test=False):
        ...
        with open(out_file, "w", newline="") as handle_out:
            ...
            for project in lst_projects:
                ...
                vect_out.append(
                    ""
                    if project.last_change_date is None
                    else project.last_change_date.strftime(
                        settings.DATETIME_FORMAT_FOR_SHOW
                    )
                )
                if not b_test:
                    vect_out.append(
                        ""
                        if project.creation_date is None
                        else project.creation_date.strftime(
                            settings.DATETIME_FORMAT_FOR_SHOW
                        )
                    )
...
class ParsePangolinResult(object):
    ...
    def get_value(self, sample_name_starts_with, value_to_call):
        if sample_name_starts_with is None:
            return ""
        vect_match = [
            key
            for key in self.dt_data.keys()
            if key.startswith(sample_name_starts_with)
        ]
        return ";".join(
            [
                self.dt_data.get(key, {}).get(value_to_call, "")
                for key in vect_match
                if len(self.dt_data.get(key, {}).get(value_to_call, "")) > 0
            ]
        )
    ...
    def process_file(self):
        ...
        if os.path.exists(self.file_pangolin_output):
            ...
            for line in vect_data:
                if not b_header:
                    if line.startswith(ParsePangolinResult.HEADER_PANGOLIN_file):
                        b_header = True
                    for i, value in enumerate(line.split(",")):
                        if value.lower() in ParsePangolinResult.VECT_CALL:
                            self.dt_header[value] = i
