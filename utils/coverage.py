...
from constants.constants import FileExtensions, FileType, TypePath
...
class DrawAllCoverage(object):
    ...
    def draw_all_coverages(
        self, project_sample, software_name=SoftwareNames.SOFTWARE_SNIPPY_name
    ):
        """
    ...
    def draw_all_coverages_explicit_input(
        ...
        vect_count_type = ["snp"]
        tab_file_from_freebayes = project_sample.get_file_output(
            TypePath.MEDIA_ROOT,
            FileType.FILE_TAB,
        ...
        if os.path.exists(tab_file_from_freebayes):
            (dict_less_50, dict_more_50, dict_more_90) = (
                utils.get_variations_by_freq_from_tab(
                    tab_file_from_freebayes, vect_count_type
                )
            )
        else:
            tab_file_from_medaka = project_sample.get_file_output(
                TypePath.MEDIA_ROOT,
                FileType.FILE_TAB,
            ...
            (dict_less_50, dict_more_50, dict_more_90) = (
                utils.get_variations_by_freq_from_tab(
                    tab_file_from_medaka, vect_count_type
                )
            )
        ...
        for sequence_name in geneticElement.get_sorted_elements():
            draw_coverage.create_coverage(
                ...
                project_sample.get_global_file_by_element(
                    TypePath.MEDIA_ROOT,
                ...
                (
                    ...
                    else coverage.get_coverage(sequence_name, Coverage.COVERAGE_MORE_9)
                ),
...
class DrawCoverage(object):
    ...
    GAP_START_GENES_Y = (
        SIZE_TOTAL_COVERAGE_Y + DRAW_COVERAGE_Y + 10
    )  ## position to draw the exons, green part
    ...
    DRAW_BOTTOM_Y = (
        GAP_START_GENES_Y
        + DRAW_HEIGHT_GENE_SQUARE_Y
        + DRAW_HEIGHT_GENE_STRAND_Y
        + (GAP_START_Y >> 1)
        + (START_DRAW_HEADER << 2)
        + 40
    )
    ...
    def __init__(self, limit_defined_by_user):
        ...
        self.limit_defined_by_user = (
            self.LIMIT_MINUNUM_COVERAGE
            if limit_defined_by_user is None
            else limit_defined_by_user
        )
    ...
    def create_coverage(
        ...
        ## size of the image
        self.maxSizeImage = (
            (self.GAP_START_GENES_X << 1) + int((len(vect_coverage) / self.rateImage)),
            self.DRAW_BOTTOM_Y,
        )

        self.start_image_x = (
            (
                DrawCoverage.GAP_START_GENES_X
                + ((DrawCoverage.DRAW_MINIMUN_LENGTH_X - self.maxSizeImage[0]) >> 1)
            )
            if self.maxSizeImage[0] < DrawCoverage.DRAW_MINIMUN_LENGTH_X
            else DrawCoverage.GAP_START_GENES_X
        )
        size_image_x = (
            DrawCoverage.DRAW_MINIMUN_LENGTH_X
            if self.maxSizeImage[0] < DrawCoverage.DRAW_MINIMUN_LENGTH_X
            else self.maxSizeImage[0]
        )
    ...
    def draw_genes_and_coverage(self, draw, vect_coverage, vect_genes):
        ...
        color_squares.paint_bar(
            (
                self.get_start_x(),
                self.GAP_START_GENES_Y + (self.SHRINK_GENE_BAR_UTR << 1) + 1,
            ),
            (
                self.get_start_x() + int(len(vect_coverage) / self.rateImage),
                (self.GAP_START_GENES_Y + self.DRAW_HEIGHT_GENE_SQUARE_Y)
                - (self.SHRINK_GENE_BAR_UTR << 1)
                - 1,
            ),
            False,
        )
        for gene in vect_genes:
            ...
            self.draw_strand(
                draw,
                1 if gene.is_forward() else -1,
                int(gene.start / self.rateImage) + self.get_start_x(),
                (int(gene.end / self.rateImage) + self.get_start_x())
                - (int(gene.start / self.rateImage) + self.get_start_x()),
                (
                    (self.GAP_START_GENES_Y + self.DRAW_HEIGHT_GENE_SQUARE_Y)
                    if gene.is_forward()
                    else self.GAP_START_GENES_Y
                ),
            )
            color_squares.draw_text(
                int(gene.start / self.rateImage) + self.get_start_x(),
                self.get_start_x() + int(gene.end / self.rateImage),
                (
                    (self.GAP_START_GENES_Y + self.DRAW_HEIGHT_GENE_SQUARE_Y)
                    if gene.is_forward()
                    else self.GAP_START_GENES_Y - self.DRAW_HEIGHT_GENE_SQUARE_Y
                ),
                gene.name,
            )
    ...
    def draw_strand(self, draw, direction, start_x, nLength_X, start_y):
        ...
        """
        draw.line(
            (start_x, start_y, start_x + nLength_X, start_y),
        ...
        if (nLength_X >> 4) > 1:
            for i in range(0, nLength_X >> 4):
                ...
                draw.point(
                    (start_x + slice_pos + direction, start_y - 2),
                    fill=self.COLOR_RGBBlack,
                )
                draw.point((start_x + slice_pos, start_y + 1), fill=self.COLOR_RGBBlack)
                draw.point(
            ...
            draw.point(
                (start_x + (nLength_X >> 1), start_y - 1), fill=self.COLOR_RGBBlack
            )
            draw.point(
                (start_x + (nLength_X >> 1) + direction, start_y - 2),
                fill=self.COLOR_RGBBlack,
            )
            draw.point(
                (start_x + (nLength_X >> 1), start_y + 1), fill=self.COLOR_RGBBlack
    ...
    def draw_coverage(self, draw, vect_coverage):
        ...
        for i in range(0, len(vect_coverage), self.rateImage):
            vect_coverage_average.append(
                int(numpy.mean(vect_coverage[i : i + self.rateImage]))
            )

        draw.line(
            (
                start_x - 1,
                self.DRAW_COVERAGE_Y,
                start_x - 1,
                self.DRAW_COVERAGE_Y + self.SIZE_COVERAGE_Y,
            ),
        ...
        draw.line(
            (
                start_x + len(vect_coverage_average),
                self.DRAW_COVERAGE_Y,
                start_x + len(vect_coverage_average),
                self.DRAW_COVERAGE_Y + self.SIZE_COVERAGE_Y,
            ),
        ...
        draw.line(
            (
                start_x - 1,
                self.DRAW_COVERAGE_Y + self.SIZE_COVERAGE_Y + 1,
                start_x + len(vect_coverage_average),
                self.DRAW_COVERAGE_Y + self.SIZE_COVERAGE_Y + 1,
            ),
            fill=self.COLOR_RGBGrey_64_64_64,
        ...
        for coverage in vect_coverage_average:
            ...
            if coverage >= (self.rateDrawCoverage * self.SIZE_COVERAGE_Y):
                ...
                draw_y = (
                    self.DRAW_COVERAGE_Y
                    + self.SIZE_COVERAGE_Y
                    - (coverage / self.rateDrawCoverage)
                )
                if draw_y == (self.DRAW_COVERAGE_Y + self.SIZE_COVERAGE_Y):
                    draw_y = posYBottom
                if coverage <= self.limit_defined_by_user:
                    bLessThanLimitLast = True

            draw.line(
                (start_x, posYBottom, start_x, draw_y),
                fill=self.COLOR_RGBGrey_225_225_225,
                width=1,
            )
            if draw_y_last == 0:
                draw.point((start_x, draw_y), fill=self.COLOR_RGBGrey_64_64_64)
            else:
                draw.line(
                    (start_x - 1, draw_y_last, start_x, draw_y),
                    fill=self.COLOR_RGBGrey_64_64_64,
                    width=1,
                )
            ...
            if bMoreThanLimitLast and draw_y_last > 0:
                draw.line(
                    (start_x - 1, draw_y_last, start_x - 1, draw_y_last - 2),
                ...
                >= (
                    self.DRAW_COVERAGE_Y
                    + self.SIZE_COVERAGE_Y
                    - (self.limit_defined_by_user / self.rateDrawCoverage)
                )
            ):
                draw.line(
                    (start_x - 1, draw_y_last, start_x - 1, draw_y_last - 2),
    ...
    def draw_header(
        self,
        draw,
        position_x_first_header,
        position_x_second_header,
        position_x_third_header,
        position_x_fourth_header,
        average_coverage,
        ratio_more_zero,
        rati_more_nine,
        ...
        if not b_only_calculate_size:
            ...
            start_coverage = (
                ...
                else (self.get_start_x() - start_coverage)
            )
            self.draw_text_header(
                draw,
                font_12,
                "Coverage".format(sample_name, sequence_name),
                start_coverage,
                self.DRAW_COVERAGE_Y - 17,
                DrawCoverage.COLOR_RGBGrey_32_32_32,
                b_only_calculate_size,
            )
        ...
        position_x += self.draw_text_header(
            draw,
            font_12,
            "Mean depth of coverage: {}".format(average_coverage),
            position_x,
            point_y,
            DrawCoverage.COLOR_RGBGrey_32_32_32,
            b_only_calculate_size,
        ...
        position_x += self.draw_text_header(
            draw,
            font_12,
            "% of size covered by at least 1-fold: {}%".format(ratio_more_zero),
            position_x,
            point_y,
            DrawCoverage.COLOR_RGBGrey_32_32_32,
            b_only_calculate_size,
        ...
        position_x += self.draw_text_header(
            draw,
            font_12,
            "Cov. <{}".format(self.limit_defined_by_user),
            position_x + lines_size,
            point_y,
            self.COLOR_RGBRed_153_0_0,
            b_only_calculate_size,
        )
        position_x += lines_size
        if not b_only_calculate_size:
        ...
        position_x += self.draw_text_header(
            draw,
            font_12,
            "Cov. >{}".format((self.rateDrawCoverage * self.SIZE_COVERAGE_Y)),
            position_x + lines_size,
            point_y,
            self.COLOR_RGBGreen_0_153_0,
            b_only_calculate_size,
        )
        ...
        if not b_only_calculate_size:
            ...
            draw.ellipse(
                ...
                fill=self.COLOR_RGBRed_153_0_0,
                outline=self.COLOR_RGBRed_153_0_0,
            )
        position_x += lines_size
        position_x += self.draw_text_header(
            draw,
    ...
    def draw_text_header(
        self, draw, font_, text, pointX, pointY, color, bOnlyCalculatesize
    ):
        gap_between_text = 20
    ...
    def draw_legend_coverage(self, draw, startDraw, endDraw, length):
        ...
        for i in range(0, int(length / number_space)):
            draw.line(
                (
                    self.get_start_x() + i * number_space,
                    self.DRAW_COVERAGE_Y + self.SIZE_COVERAGE_Y,
                    self.get_start_x() + i * number_space,
                    self.DRAW_COVERAGE_Y + self.SIZE_COVERAGE_Y + tick_length,
                ),
                fill=self.COLOR_RGBGrey_64_64_64,
                width=1,
            )
            ...
            draw.text(
                ...
                "{}".format(i * number_space * self.rateImage),
                fill=DrawCoverage.COLOR_RGBGrey_32_32_32,
                font=font_,
            )

        smallOffset = 4
        self.draw_legend_coverage_and_text(
            draw,
            font_,
            startDraw,
            endDraw,
            self.DRAW_COVERAGE_Y + smallOffset,
            "{}".format(self.SIZE_COVERAGE_Y * self.rateDrawCoverage),
        )
        self.draw_legend_coverage_and_text(
            draw,
            font_,
            startDraw,
            endDraw,
            self.DRAW_COVERAGE_Y + (self.SIZE_COVERAGE_Y >> 2) + smallOffset,
            "{}".format(int(((self.SIZE_COVERAGE_Y * self.rateDrawCoverage) / 4) * 3)),
        )
        self.draw_legend_coverage_and_text(
            draw,
            font_,
            startDraw,
            endDraw,
            self.DRAW_COVERAGE_Y + (self.SIZE_COVERAGE_Y >> 1) + smallOffset,
            "{}".format((self.SIZE_COVERAGE_Y * self.rateDrawCoverage) >> 1),
        )
        self.draw_legend_coverage_and_text(
            draw,
            font_,
            startDraw,
            endDraw,
            self.DRAW_COVERAGE_Y + ((self.SIZE_COVERAGE_Y >> 2) * 3) + smallOffset,
            "{}".format(int((self.SIZE_COVERAGE_Y * self.rateDrawCoverage) / 4)),
        )
    ...
    def draw_legend_coverage_and_text(
        ...
        if step > 1:
            ...
            for i in range(0, step - 1):
                draw.text(
                    (self.get_start_x() + (i + 1) * slice_, pointY + smallOffset_y),
                    value,
                    fill=DrawCoverage.COLOR_RGBGrey_32_32_32,
                    font=font_,
                )
            ...
            draw.text(
                (self.get_start_x() + (nLength_X >> 1), pointY + smallOffset_y),
                value,
                fill=DrawCoverage.COLOR_RGBGrey_32_32_32,
                font=font_,
            )
    ...
    def draw_variant(self, draw, position, b_up):
        if b_up:
            draw.line(
                (
                    self.get_start_x() + position,
                    self.GAP_START_GENES_Y - self.GAP_MARK_VARIATIONS,
                    self.get_start_x() + position,
                    self.GAP_START_GENES_Y
                    + self.DRAW_HEIGHT_GENE_SQUARE_Y
                    - (self.GAP_MARK_VARIATIONS << 1),
                ),
                fill=self.COLOR_RGBRed_153_0_0,
                width=1,
            )
            draw.ellipse(
                (
                    self.get_start_x() + position - 3,
                    self.GAP_START_GENES_Y - (self.GAP_MARK_VARIATIONS * 3),
                    self.get_start_x() + position + 3,
                    self.GAP_START_GENES_Y - self.GAP_MARK_VARIATIONS,
                ),
                fill=self.COLOR_RGBRed_153_0_0,
            ...
            draw.line(
                (
                    self.get_start_x() + position,
                    self.GAP_START_GENES_Y + (self.GAP_MARK_VARIATIONS << 1),
                    self.get_start_x() + position,
                    self.GAP_START_GENES_Y
                    + self.DRAW_HEIGHT_GENE_SQUARE_Y
                    + self.GAP_MARK_VARIATIONS,
...
class ColorSquares(object):
    ...
    def set_colors(self):
        ...
        self.m_crShadow = (
            byRed3DLiteShadow + ((byRed3DDkShadow - byRed3DLiteShadow) >> 1),
    ...
    def lighten_color(self, cr_color, by_increase_val):
        ...
        if (byRed + by_increase_val) <= 255:
            byRed = byRed + by_increase_val
        if (byGreen + by_increase_val) <= 255:
            byGreen = byGreen + by_increase_val
        if (byBlue + by_increase_val) <= 255:
            byBlue = byBlue + by_increase_val
    ...
    def darken_color(self, crColor, byReduceVal):
        ...
        if byRed >= byReduceVal:
            byRed = byRed - byReduceVal
        if byGreen >= byReduceVal:
            byGreen = byGreen - byReduceVal
        if byBlue >= byReduceVal:
            byBlue = byBlue - byReduceVal
    ...
    def paint_bar(self, rect_point_top, rect_point_bottom, b_vertical):
        if b_vertical:
            self.draw.rectangle(
                (rect_point_top, rect_point_bottom), fill=self.m_crColorLightest
            )
        else:
            self.draw.rectangle(
                (
                    ...
                    rect_point_top[1],
                    rect_point_bottom[0],
                    rect_point_bottom[1] - 1,
                ),
                fill=self.m_crColorLightest,
            )

        if b_vertical:
            self.draw_vertical_bar(rect_point_top, rect_point_bottom)
        else:
            self.draw_horizontal_bar(rect_point_top, rect_point_bottom)
    ...
    def draw_horizontal_bar(self, rect_point_top, rect_point_bottom):
        ...
        if height > 7:
            self.draw.line(
                (nLeft + 2, nBottom - 4, nRight - 1, nBottom - 4),
                fill=self.m_crColorLight,
                width=1,
            )
            self.draw.line(
                (nLeft + 1, nTop + 2, nRight - 1, nTop + 2),
                fill=self.m_crColorLight,
                width=1,
            )
            ...
            self.draw.line(
                (nLeft + 2, nBottom - 5, nRight - 2, nBottom - 5),
                fill=self.m_crColorLighter,
                width=1,
            )
            self.draw.line(
                (nRight - 2, nBottom - 5, nRight - 2, nTop + 3),
                fill=self.m_crColorLighter,
                width=1,
            )
            self.draw.line(
                (nRight - 2, nTop + 3, nLeft + 1, nTop + 3),
                fill=self.m_crColorLighter,
                width=1,
            )
            ...
            self.draw.line(
                (nLeft, nBottom - 1, nLeft, nTop), fill=self.m_crColor, width=1
            )
            self.draw.line((nLeft, nTop, nLeft + 2, nTop), fill=self.m_crColor, width=1)
            self.draw.line(
                (nLeft + 1, nBottom - 3, nRight - 1, nBottom - 3),
                fill=self.m_crColor,
                width=1,
            )
            ...
            self.draw.line(
                (nLeft + 1, nBottom - 2, nRight - 1, nBottom - 2),
                fill=self.m_crColorDark,
                width=1,
            )
            self.draw.line(
                (nRight - 1, nBottom - 2, nRight - 1, nTop + 1),
                fill=self.m_crColorDark,
                width=1,
            )
            self.draw.line(
                (nLeft + 1, nTop, nRight, nTop), fill=self.m_crColorDark, width=1
            )
            self.draw.point((nLeft + 1, nBottom - 1), fill=self.m_crColorDark)

            self.draw.line(
                (nLeft + 1, nBottom - 1, nRight, nBottom - 1),
                fill=self.m_crColorDarker,
                width=1,
            )
            self.draw.line(
                (nRight, nBottom - 1, nRight, nTop), fill=self.m_crColorDarker, width=1
            )
        else:
    ...
    def draw_vertical_bar(self, rect_point_top, rect_point_bottom):
        ...
        if height > 7:
            ...
            self.draw.line(
                (nLeft + 1, nBottom - 2, nLeft + 1, nTop + 1),
                fill=self.m_crColor,
                width=1,
            )
            self.draw.line(
                (nRight - 2, nBottom - 3, nRight - 2, nTop + 1),
                fill=self.m_crColor,
                width=1,
            )
            ...
            self.draw.line(
                (nLeft + 2, nBottom - 3, nLeft + 2, nTop + 1),
                fill=self.m_crColorLight,
                width=1,
            )
            self.draw.line(
                (nRight - 3, nBottom - 3, nRight - 3, nTop + 1),
                fill=self.m_crColorLight,
                width=1,
            )
            self.draw.point((nLeft + 1, nTop + 1), fill=self.m_crColorLight)
            ...
            self.draw.line(
                (nLeft + 3, nBottom - 3, nLeft + 3, nTop + 1),
                fill=self.m_crColorLighter,
                width=1,
            )
            self.draw.line(
                (nRight - 4, nBottom - 3, nRight - 4, nTop + 1),
                fill=self.m_crColorLighter,
                width=1,
            )
            ...
            self.draw.line(
                (nLeft, nBottom - 1, nLeft, nTop + 1), fill=self.m_crColorDark, width=1
            )
            self.draw.line(
                (nLeft + 2, nBottom - 2, nRight - 1, nBottom - 2),
                fill=self.m_crColorDark,
                width=1,
            )
            self.draw.line(
            ...
            self.draw.line(
                (nRight, nBottom - 1, nRight, nTop + 1),
                fill=self.m_crColorDarker,
                width=1,
            )
        else:
    ...
    def paintBarsEx(
        ...
        if bVertical:
            ...
            rect_point_top_temp = (
                rect_point_top[0],
                rect_point_top[1] + (shrinkFirst << 1),
            )
        ...
        if bVertical:
            ...
            rect_point_bottom_temp = (
                rect_point_bottom[0],
                rect_point_bottom[1] - (shrinkLast << 1),
            )
        self.set_color(rgb2)
        self.paint_bar(rect_point_top_temp, rect_point_bottom_temp, bVertical)
