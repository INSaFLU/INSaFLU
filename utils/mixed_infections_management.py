...
class MixedInfectionsManagement(object):
    ...
    def get_mixed_infections_empty_value(self):
        ...
        lst_mixed_infections = MixedInfections.objects.filter(
            ...
            average_value=0.0,
            description="{}",
            has_master_vector=False,
        ).order_by("-pk")[:1]
        if len(lst_mixed_infections) > 0:
            return lst_mixed_infections[0]
    ...
    def get_mixed_infections(self, project_sample, user, count_hits):
        ...
        if project_sample.sample.is_type_fastq_gz_sequencing():
            ...
            meta_data_sample_mixed_infection = manage_database.get_sample_metakey_last(
                project_sample.sample,
                MetaKeyAndValue.META_KEY_ALERT_MIXED_INFECTION_TYPE_SUBTYPE,
                MetaKeyAndValue.META_VALUE_Success,
            )
            meta_data_tag_sample_mixed_infection = (
                manage_database.get_sample_metakey_last(
                    project_sample.sample,
                    MetaKeyAndValue.META_KEY_TAG_MIXED_INFECTION_TYPE_SUBTYPE,
                    MetaKeyAndValue.META_VALUE_Success,
                )
            )
            if not meta_data_tag_sample_mixed_infection is None:
                tag = meta_data_tag_sample_mixed_infection.description
            else:
                (tag, alert, message) = project_sample.sample.get_mixed_infection()

            ## to be sure that is NO or YES
            if (
                tag != ConstantsMixedInfection.TAGS_MIXED_INFECTION_YES
                and tag != ConstantsMixedInfection.TAGS_MIXED_INFECTION_NO
            ):
                tag = ConstantsMixedInfection.TAGS_MIXED_INFECTION_NO

            ## test ratio methodm only for illumina
            if (
                count_hits.is_mixed_infection_ratio_test()
                or count_hits.total_grather_than_mixed_infection()
            ):  ## doesn't matter the other
                tag = ConstantsMixedInfection.TAGS_MIXED_INFECTION_YES
        else:
            value = 0.0
            tag = Constants.EMPTY_VALUE_NA
        ...
        mixed_infections.description = (
            self.get_mixed_infection_main_vector().to_json()
            if project_sample.sample.is_type_fastq_gz_sequencing()
            else "{}"
        )
        mixed_infections.save()
        ...
        if project_sample.sample.is_type_fastq_gz_sequencing() and (
            tag == ConstantsMixedInfection.TAGS_MIXED_INFECTION_YES
            or count_hits.is_mixed_infection_ratio_test()
            or count_hits.total_grather_than_mixed_infection()
        ):
            ...
            if count_hits.is_mixed_infection_ratio_test():
                manage_database.set_project_sample_metakey(
                    ...
                    MetaKeyAndValue.META_VALUE_Success,
                    "Warning: this sample has a ratio of the number of iSNVs at frequency 1-50% (minor iSNVs) "
                    + "and 50-90% of '{}' (within the range 0.5-2.0) and a total number of iSNVs from the two categories of '{}' ".format(
                        count_hits.get_mixed_infection_ratio_str(),
                        count_hits.get_total_50_50_90(),
                    )
                ...
                manage_database.set_project_sample_metakey(
                    ...
                    MetaKeyAndValue.META_VALUE_Success,
                    "Warning: this sample has a sum of the number of iSNVs at frequency 1-50% (minor iSNVs) "
                    + "and 50-90% of '{}' (i.e., above {}) suggesting that may represent a 'mixed infection'.".format(
                        count_hits.get_total_50_50_90(),
                        count_hits.TOTAL_GRATHER_THAN_MIXED,
                    ),
                )
            ...
            if meta_data_sample_mixed_infection != None:
                manage_database.set_project_sample_metakey(
                    ...
                    MetaKeyAndValue.META_KEY_ALERT_MIXED_INFECTION_TYPE_SUBTYPE,
                    MetaKeyAndValue.META_VALUE_Success,
                    meta_data_sample_mixed_infection.description,
                )
    ...
    def get_mixed_infection_main_vector(self):
        ...
        """
        try:
            mixed_infections_main_vector = MixedInfections.objects.get(
                has_master_vector=True
            )
            ...
            mixed_infections_main_vector.description = (
                mixed_infection_main_vector.to_json()
            )
