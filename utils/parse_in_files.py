...
class ParseInFiles(object):
    ...
    STATE_READ_all = "all"
    STATE_READ_metadata_only_detect_errors_and_chech_samples = (
        "metadata_only_detect_errors_check_samples"
    )
    STATE_READ_metadata_dont_detect_errors_and_chech_samples = (
        "metadata_dont_detect_errors_check_samples"
    )

    ## only for metadata
    vect_only_read_metadata = [
        STATE_READ_metadata_only_detect_errors_and_chech_samples,
        STATE_READ_metadata_dont_detect_errors_and_chech_samples,
    ]
    ...
    vect_header = [
        "sample name",
        TAG_SAMPLE_NAME_fastq1,
        TAG_SAMPLE_NAME_fastq2,
        TAG_SAMPLE_NAME_data_set,
        TAG_SAMPLE_NAME_vaccine_status,
        TAG_SAMPLE_NAME_week,
        TAG_SAMPLE_NAME_onset_date,
        TAG_SAMPLE_NAME_collection_date,
        TAG_SAMPLE_NAME_lab_reception_date,
        TAG_SAMPLE_NAME_latitude,
        TAG_SAMPLE_NAME_longitude,
    ]
    ...
    def clean_data(self):
        ...
        self.vect_samples = (
            []
        )  ## [[sample, vect_tag_names], [sample1, vect_tag_names1], ... ]
    ...
    def parse_sample_files(self, file_name, user, b_test_char_encoding, read_state):
        ...
        try:
            ...
            self.errors.add_single_result(
                SingleResult(SingleResult.ERROR, "csv or tsv file. " + str(e))
            )
    ...
    def process_file(self, f, user, read_state):
        ...
        for line in f:
            ...
            if delimiter in dict_delimiter:
                dict_delimiter[delimiter] += 1
            else:
                dict_delimiter[delimiter] = 1
            if n_lines > 20:
                break
        ...
        for key in dict_delimiter:
            ...
            for row in reader:
                ...
                for col in row:
                    if (
                        count_column >= len(self.vect_header)
                        or col.replace(" ", "").lower()
                        != self.vect_header[count_column].replace(" ", "").lower()
                    ):
                        break
                    n_columns_ok += 1
                    count_column += 1
        ...
        for row in reader:
            if header == None:
                if (
                    read_state in ParseInFiles.vect_only_read_metadata
                ):  ## only for metadata
                    count_column = 0
                    n_columns_ok = 0
                    for col in row:
                        if (
                            count_column
                            >= len(ParseInFiles.vect_madatory_header_metadata)
                            or col.replace(" ", "").lower()
                            != ParseInFiles.vect_madatory_header_metadata[count_column]
                            .replace(" ", "")
                            .lower()
                        ):
                            break
                            ...
                            if row[i].strip() in self.dict_other_fields_repeated:
                                self.errors.add_single_result(
                                    SingleResult(
                                        SingleResult.ERROR,
                                        _(
                                            "Column name '{}' is repeated in the header. Line: {} Column: {}".format(
                                                row[i].strip(), count_row, i + 1
                                            )
                                        ),
                                    )
                                )
        ...
        if header is None:
            self.errors.add_single_result(
                SingleResult(
                    SingleResult.ERROR,
                    _(
                        "Header not found in the file. Please, check the names in the header, must be equal and have the same order of the template file."
                    ),
                )
            )
            ...
            if read_state in ParseInFiles.vect_only_read_metadata:
                self.errors.add_single_result(
                    SingleResult(SingleResult.ERROR, _("There's no samples to update."))
                )
            else:
                self.errors.add_single_result(
                    SingleResult(
                        SingleResult.ERROR, _("There's no samples to process.")
                    )
                )
    ...
    def process_row(self, row, count_row, header, user, read_state):
        ...
        if read_state != ParseInFiles.STATE_READ_dont_detect_errors:
            if len(row) > 0 and len(row[0].strip()) > 0:
                ...
                if len(result_filer_sample_name) != len(sample_name):
                    self.errors.add_single_result(
                        SingleResult(
                            SingleResult.ERROR,
                            _(
                                "Sample name '{}' only letters, numbers and underscores are allowed. Line: {} Column: {}".format(
                                    sample_name, count_row, 1
                                )
                            ),
                        )
                    )
                    ...
                    try:
                        sample = Sample.objects.get(
                            name__iexact=sample_name, owner=user, is_deleted=False
                        )
                        self.errors.add_single_result(
                            SingleResult(
                                SingleResult.ERROR,
                                _(
                                    "Sample name '{}' exists in database. Line: {} Column: {}".format(
                                        sample_name, count_row, 1
                                    )
                                ),
                            )
                    ...
                    if sample_name in self.dict_samples_out:
                        self.errors.add_single_result(
                            SingleResult(
                                SingleResult.ERROR,
                                _(
                                    "Sample name '{}' is repeated in the file. Line: {} Column: {}".format(
                                        sample_name, count_row, 1
                                    )
                                ),
                            )
                ...
                self.errors.add_single_result(
                    SingleResult(
                        SingleResult.ERROR,
                        _(
                            "There's no sample name Line: {} Column: {}".format(
                                count_row, 1
                            )
                        ),
                    )
            ...
            if len(row) > 1:
                ...
                if not fastq1.strip().endswith(FileExtensions.FILE_GZ):
                    self.errors.add_single_result(
                        SingleResult(
                            SingleResult.ERROR,
                            _(
                                "File '{}' not ends with '{}'. Fastq gzip compressed file is needed. Line: {} Column: {}".format(
                                    fastq1.strip(), FileExtensions.FILE_GZ, count_row, 2
                                )
                            ),
                        )
                    )
                    ...
                    self.errors.add_single_result(
                        SingleResult(
                            SingleResult.ERROR,
                            _(
                                "File '{}' is repeated in the samples file. Line: {} Column: {}".format(
                                    fastq1.strip(), count_row, 2
                                )
                            ),
                        )
                    )
                ...
                self.errors.add_single_result(
                    SingleResult(
                        SingleResult.ERROR,
                        _(
                            "There's no fastq1 file name. Line: {} Column: {}".format(
                                count_row, 2
                            )
                        ),
                    )
                ...
                if not fastq2.endswith(FileExtensions.FILE_GZ):
                    self.errors.add_single_result(
                        SingleResult(
                            ...
                            _(
                                "File '' not ends with '{}'. Fastq gzip compressed file is needed. Line: {} Column: {}".format(
                                    fastq2, FileExtensions.FILE_GZ, count_row, 3
                                )
                            ),
                        )
                    )
                ...
                if len(week) > 0 and not self.utils.is_integer(week):
                    self.errors.add_single_result(
                        SingleResult(
                            SingleResult.ERROR,
                            _(
                                "'week' must be integer. Line: {} Column: {}".format(
                                    count_row, 3
                                )
                            ),
                        )
            ...
            self.validate_date(
                row, 6, count_row, self.vect_header
            )  ## validate onset date
            self.validate_date(
                row, 7, count_row, self.vect_header
            )  ## validate collection date
            self.validate_date(
                row, 8, count_row, self.vect_header
            )  ## validate lab reception date

            ### latitude
            if len(row) > 9 and len(row[9].strip()) > 0:
                latitude = row[9].strip()
                if (
                    not self.utils.is_float(latitude)
                    ...
                    self.errors.add_single_result(
                        SingleResult(
                            SingleResult.ERROR,
                            _(
                                "'latitude' must have values between -90&ltlat&lt90. Line: {} Column: {}".format(
                                    count_row, 3
                                )
                            ),
                        )
            ...
            if len(row) > 10 and len(row[10].strip()) > 0:
                ...
                if (
                    not self.utils.is_float(longitude)
                    ...
                    self.errors.add_single_result(
                        SingleResult(
                            SingleResult.ERROR,
                            _(
                                "'longitude' must have values between -180&ltlong&lt180. Line: {} Column: {}".format(
                                    count_row, 3
                                )
                            ),
                        )
        ...
        if n_errors == len(self.errors.get_vect_results()) and (
            read_state == ParseInFiles.STATE_READ_all
            or read_state == ParseInFiles.STATE_READ_dont_detect_errors
        ):
            ...
            if len(row[2].strip()) > 0:
                sample.candidate_file_name_2 = row[2].strip()
            sample.data_set = DataSet()
            sample.data_set.owner = user
            if len(row) > 3 and len(row[3].strip()) > 0:
                sample.data_set.name = row[3].strip()
            else:
                sample.data_set.name = Constants.DATA_SET_GENERIC
            ...
            if len(row) > 5 and len(row[5].strip()) > 0:
                sample.week = int(row[5].strip())

            ## dates
            if len(row) > 6 and len(row[6].strip()) > 0:
                sample.date_of_onset = self.utils.validate_date(row[6].strip())
            if len(row) > 7 and len(row[7].strip()) > 0:
                sample.date_of_collection = self.utils.validate_date(row[7].strip())
            if len(row) > 8 and len(row[8].strip()) > 0:
                sample.date_of_receipt_lab = self.utils.validate_date(row[8].strip())
            ...
            vect_tag_names = (
                []
            )  ## this trick need to be done because 'Extra fields on many-to-many'
        ...
        if (
                    ...
                    try:
                        ...
                        self.errors.add_single_result(
                            SingleResult(
                                SingleResult.ERROR,
                                _(
                                    "Sample name '{}' doesn't exists in database. Line: {} Column: {}".format(
                                        sample_name, count_row, 1
                                    )
                                ),
                            )
            ...
            if len(row) > 1:
                for i in range(1, len(header)):
                    if (
                        header[i].strip().lower() == ParseInFiles.TAG_SAMPLE_NAME_fastq1
                        or header[i].strip().lower()
                        == ParseInFiles.TAG_SAMPLE_NAME_fastq2
                    ):
                        continue  ## don't change the file names
                    elif (
                        header[i].strip().lower()
                        == ParseInFiles.TAG_SAMPLE_NAME_data_set
                    ):
                        continue  ## don't test this field
                    elif (
                        header[i].strip().lower()
                        == ParseInFiles.TAG_SAMPLE_NAME_vaccine_status
                    ):
                        continue  ## don't test this field
                        ...
                        header[i].strip().lower()
                        == ParseInFiles.TAG_SAMPLE_NAME_onset_date
                    ):
                        self.validate_date(
                            row, i, count_row, header
                        )  ## validate onset date
                    elif (
                        header[i].strip().lower()
                        == ParseInFiles.TAG_SAMPLE_NAME_collection_date
                    ):
                        self.validate_date(
                            row, i, count_row, header
                        )  ## validate onset date
                        ...
                        header[i].strip().lower()
                        == ParseInFiles.TAG_SAMPLE_NAME_lab_reception_date
                    ):
                        self.validate_date(
                            row, i, count_row, header
                        )  ## validate onset date
                        ...
                        header[i].strip().lower()
                        == ParseInFiles.TAG_SAMPLE_NAME_latitude
                    ):
                        latitude = row[i].strip()
                        if len(latitude) > 0 and (
                            not self.utils.is_float(latitude)
                            or float(latitude) > 90
                            or float(latitude) < -90
                        ):
                        ...
                        header[i].strip().lower()
                        == ParseInFiles.TAG_SAMPLE_NAME_longitude
                    ):
                        longitude = row[i].strip()
                        if len(longitude) > 0 and (
                            not self.utils.is_float(longitude)
                            or float(longitude) > 180
                            or float(longitude) < -180
                        ):
                ...
                self.errors.add_single_result(
                    SingleResult(
                        SingleResult.ERROR,
                        _(
                            "There's no data to process Line: {} Column: {}".format(
                                count_row, 1
                            )
                        ),
                    )
            ...
            if len(row) > 1:
                ...
                for i in range(1, len(header)):
                    ...
                    if (
                        ...
                        if len(row[i].strip()) > 0:
                            sample.date_of_onset = self.utils.validate_date(
                                row[i].strip()
                            )
                    elif (
                        header[i].strip().lower()
                        ...
                        if len(row[i].strip()) > 0:
                            sample.date_of_collection = self.utils.validate_date(
                                row[i].strip()
                            )
                    elif (
                        header[i].strip().lower()
                        ...
                        if len(row[i].strip()) > 0:
                            sample.date_of_receipt_lab = self.utils.validate_date(
                                row[i].strip()
                            )
                    elif (
                        header[i].strip().lower()
            ...
            try:
                ...
                self.errors.add_single_result(
                    SingleResult(
                        SingleResult.ERROR,
                        _(
                            "The '{}' must have this format DD/MM/YYYY. Line: {} Column: {}".format(
                                vect_header[column], count_row, column + 1
                            )
                        ),
                    )
                )
    ...
    def create_samples(self, upload_files, user):
        ...
        if self.errors.get_len_vect_results() > 0:
            return
        ...
        for vect_sample in self.vect_samples:
            ...
            if vect_sample[0].data_set != None:
                try:
                    data_set = DataSet.objects.get(
                        name__iexact=vect_sample[0].data_set.name, owner=user
                    )
            ...
            if vect_sample[0].vaccine_status != None:
                try:
                    vaccine_status = VaccineStatus.objects.get(
                        name__iexact=vect_sample[0].vaccine_status.name, owner=user
                    )
            ...
            for tag_samples_temp in vect_sample[1]:
                try:
                    tag_name = TagName.objects.get(
                        name__iexact=tag_samples_temp.tag_name.name, owner=user
                    )
                ...
                tag_names = TagNames(
                    tag_name=tag_name, sample=sample, value=tag_samples_temp.value
                )
                tag_names.save()
    ...
    def update_samples(self, upload_files, user):
        ...
        for vect_sample in self.vect_samples:
            ...
            sample.data_set = data_set
            sample.vaccine_status = vaccine_status
            if not vect_sample[0].week is None:
                sample.week = vect_sample[0].week
            if not vect_sample[0].date_of_onset is None:
                sample.date_of_onset = vect_sample[0].date_of_onset
            if not vect_sample[0].date_of_collection is None:
                sample.date_of_collection = vect_sample[0].date_of_collection
            if not vect_sample[0].date_of_receipt_lab is None:
                sample.date_of_receipt_lab = vect_sample[0].date_of_receipt_lab
            if not vect_sample[0].geo_local is None:
                sample.geo_local = vect_sample[0].geo_local
            ...
            query_set = ProjectSample.objects.filter(
                sample=sample, is_deleted=False, is_error=False
            )
        ...
        upload_files.number_files_to_process = len(self.vect_samples)
        upload_files.number_files_processed = len(
            self.vect_samples
        )  ## has the number of samples changed
    ...
    def has_samples_files_to_process(
        self, user, type_sample=TypeFile.TYPE_FILE_sample_file
    ):
        """
    ...
    def get_upload_samples_file(self, user, type_sample=TypeFile.TYPE_FILE_sample_file):
        ...
        """
        upload_files = UploadFiles.objects.filter(
            owner=user,
            is_valid=True,
            is_deleted=False,
            is_processed=False,
    ...
    def _link_files(self, user, b_testing=False):
        ...
        for sample in upload_files.samples.all():
            ...
            if len(sample.candidate_file_name_1) > 0:
                ...
                try:
                    ...
                    upload_files_1 = UploadFiles.objects.get(
                        file_name=sample.candidate_file_name_1,
                        owner=user,
                        is_processed=False,
                        is_deleted=False,
                        is_valid=True,
                    ...
                    path_to_file = os.path.join(
                        ...
                        upload_files_1.path_name.name,
                    )

                    if (
                        not os.path.exists(path_to_file)
                        or not utils.is_fastq_gz(path_to_file)[0]
                    ):
                        upload_files_1 = None
                    else:  ## get type of sequencing
                        type_sequence_file_upload_file_1 = utils.get_type_file(
                            path_to_file
                        )
            ...
            if not upload_files_1 is None and len(sample.candidate_file_name_2) > 0:
                try:
                    upload_files_2 = UploadFiles.objects.get(
                        file_name=sample.candidate_file_name_2,
                        owner=user,
                        is_processed=False,
                        is_deleted=False,
                        is_valid=True,
                    ...
                    if (
                        ...
                        type_sequence_file_upload_file_2 = utils.get_type_file(
                            path_to_file
                        )
            ...
            ## can set the data
            if (
                len(sample.candidate_file_name_2) > 0
                and not upload_files_1 is None
                and not upload_files_2 is None
            ) or (
                ...
                if sample.type_of_fastq == Sample.TYPE_OF_FASTQ_not_defined:
                    sample.set_type_of_fastq_sequencing(
                        type_sequence_file_upload_file_1
                    )
                ...
                if (
                    not upload_files_2 is None
                    and type_sequence_file_upload_file_1
                    == Constants.FORMAT_FASTQ_illumina
                    and type_sequence_file_upload_file_2
                    == Constants.FORMAT_FASTQ_illumina
                ):
        ...
        if upload_files.number_files_processed == upload_files.number_files_to_process:
    ...
    def read_sample_file(self, user, upload_files, b_testing=False):
        ...
        try:
            ...
            parse_in_files.parse_sample_files(
                upload_files.get_path_to_file(TypePath.MEDIA_ROOT),
                user,
            ...
            parse_in_files._link_files(
                user, b_testing
            )  ### without passing the messages
        except:
            ## finished with error
            process_SGE.set_process_controler(
                user,
        ...
        try:
            ...
            if len(vect_project_affected) > 0:
                for project in vect_project_affected:
                    taskID = process_SGE.set_collect_global_files_for_update_metadata(
                        project, user
                    )
