...
from datasets.models import Consensus, UploadFiles
from managing_files.models import Reference, Sample
from utils.data_columns import (
    DataColumns,
    NEXTSTRAIN_age,
    NEXTSTRAIN_date,
...
class ParseNextStrainFiles(object):
    ...
    ## for nextstrain
    STATE_READ_metadata_only_detect_errors_and_chech_nexttrain = (
        "metadata_only_detect_errors_check_nexttrain"
    )
    STATE_READ_metadata_dont_detect_errors_and_chech_nexttrain = (
        "metadata_dont_detect_errors_check_nexttrain"
    )
    ...
    def process_file(self, f, user, read_state):
        ...
        for key in dict_delimiter:
            ...
            for row in reader:
                ...
                (header, dict_repeated) = (
                    data_columns.get_type_header_nextstrain_and_check_repeated(row)
                )
        ...
        for row in reader:
            if header is None:
                    ...
                    for repeated_column in dict_repeated:
                        self.errors.add_single_result(
                            SingleResult(
                                SingleResult.ERROR,
                                _(
                                    "Column name '{}' is repeated in the header. Line: {}".format(
                                        repeated_column, count_row
                                    )
                                ),
                            )
                        )
                ...
                self.process_row_metadata(
                    row, count_row, self.vect_header, user, read_state
                )
            count_row += 1
        if header is None:
            self.errors.add_single_result(
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    try:
                                        reference = Reference.objects.get(
                                            name__iexact=sample_name,
                                            owner__username=Constants.DEFAULT_USER,
                                            is_deleted=False,
                                        )
                                    except Reference.DoesNotExist as error:
                                        self.errors.add_single_result(
                                            SingleResult(
                                                SingleResult.ERROR,
                                                _(
                                                    "Name '{}' doesn't exists in database. Line: {} Column: {}".format(
                ...
                if sample_name in self.dict_samples_out:
                    self.errors.add_single_result(
                        SingleResult(
                            SingleResult.ERROR,
                            _(
                                "Name '{}' is repeated in the file. Line: {} Column: {}".format(
                                    sample_name, count_row, 1
                                )
                            ),
                        )
                ...
                for i in range(1, len(header)):
                    if header[i].strip().lower() == NEXTSTRAIN_length:
                        if (
                            row[i] != "?"
                            and len(row[i]) > 0
                            and not self.utils.is_integer(row[i])
                        ):
                            self.errors.add_single_result(
                                SingleResult(
                                    SingleResult.ERROR,
                                    _(
                                        "'{}' must be integer. Line: {} Column: {}".format(
                                            NEXTSTRAIN_length, count_row, i + 1
                                        )
                                    ),
                                )
                            )
    ...
    def get_value(self, sample_name, header_name):
        ...
        """
        if not header_name in self.dict_header:
            return None
        if not sample_name in self.dict_samples:
            return None
        index = self.dict_header[header_name]
        if index < len(self.dict_samples[sample_name]):
            return self.dict_samples[sample_name][index]
        return None
    ...
    def validate_date(self, row, column, count_row, vect_header):
        ...
        if len(row[column].strip()) > 0 and row[column].strip() != "?":
            try:
                return self.utils.validate_date_format(
                    row[column].strip(), "%Y-%m-%d", "YYYY-mm-dd"
                )
                ...
                self.errors.add_single_result(
                    SingleResult(
                        SingleResult.ERROR,
                        _(
                            "The '{}' must have this format YYYY-MM-DD. Line: {} Column: {}".format(
