...
class DecodeObjects(object):
    ...
    def decode_object(self, o):
        if "__Coverage__" in o:
            ...
            a = FeatureLocationSimple(
                o["__FeatureLocationSimple__"]["start"],
                o["__FeatureLocationSimple__"]["end"],
            ...
            a = SoftwareDesc(
                o["__software_desc__"]["name"],
                o["__software_desc__"]["version"],
                o["__software_desc__"]["parameters"],
            )
            ...
            a = SingleResult(
                o["__SingleResult__"]["result"], o["__SingleResult__"]["message"]
            )
            ...
            a = ResultAverageAndNumberReads(
                o["__ResultAverageAndNumberReads__"]["number_file_1"],
                o["__ResultAverageAndNumberReads__"]["average_file_1"],
                o["__ResultAverageAndNumberReads__"]["number_file_2"],
...
class Softwares(object):
    ...
    def get_software(self, sz_name, b_not_add_software_name=False):
        ...
        for software_desc in self.list_software:
            if software_desc.name.lower().startswith(sz_name.lower()):
                version_data = ""
                ## version
                if len(software_desc.version) > 0:
                    version_data = "-{}".format(software_desc.version)
                ###
                if (
                    not software_desc.parameters is None
                    and len(software_desc.parameters) > 0
                ):
                    if b_not_add_software_name:
                        description = "{}".format(software_desc.parameters)
                    else:
                        description = "{}{}; ({})".format(
                            software_desc.name, version_data, software_desc.parameters
                        )
                    if not description in list_return:
                        list_return.append(description)
                else:
    ...
    def get_list_software_instance(self, sz_name):
        ...
        for software_desc in self.list_software:
            if software_desc.name == sz_name:
                vect_software.append(software_desc)
    ...
    def get_all_software_names(self):
        ...
        for software in self.list_software:
            if not software.name in vect_out:
                vect_out.append(software.name)
        return vect_out
...
class ResultAverageAndNumberReads(object):
    ...
    def __eq__(self, other):
        return (
            other != None
            and other.number_file_1 == self.number_file_1
            and other.average_file_1 == self.average_file_1
            and other.number_file_2 == self.number_file_2
            and other.average_file_2 == self.average_file_2
        )
    ...
    def has_reads(self):
        ...
        if self.number_file_1 == "0" and (
            self.number_file_2 == "0" or self.number_file_2 is None
        ):
            return False
        return True
...
class Coverage(object):
    ...
    def __init__(self, limit_defined_by_user=None, limit_defined_to_project=None):
        self.limit_defined_by_user = (
            limit_defined_by_user  ### if there is limit defined by user put it here
        )
        ## limit_defined_by_user is called first
        self.limit_defined_to_project = (
            limit_defined_to_project  ### if there is limit defined to the project
        )
        self.dt_data = {}
    ...
    def get_middle_limit(self):
        ...
        if not self.limit_defined_by_user is None:
            return self.limit_defined_by_user
        if not self.limit_defined_to_project is None:
            return self.limit_defined_to_project
        return 10
    ...
    def get_type_coverage_middle_limit(self):
        ...
        """
        if not self.limit_defined_by_user is None:
            return self.COVERAGE_MORE_DEFINED_BY_USER
        if not self.limit_defined_to_project is None:
            return self.COVERAGE_PROJECT
        return self.COVERAGE_MORE_9

    def get_dict_data(self):
        return self.dt_data

    def get_sorted_elements_name(self):
        return sorted(self.dt_data.keys())
    ...
    def ratio_value_coverage_bigger_limit(self, element, limit_to_mask_consensus):
        ...
        if (
            self.ratio_value_project != 0
            and self.ratio_value_project > limit_to_mask_consensus
        ):
            return True
        if (
            self.ratio_value_defined_by_user != 0
            and self.ratio_value_defined_by_user > limit_to_mask_consensus
        ):
            return True
        if self.ratio_value_9 != 0 and self.ratio_value_9 > limit_to_mask_consensus:
            return True
        return False
    ...
    def __str__(self):
        ...
        for key in self.dt_data:
            sz_return += "{} - All {}; 9- {}; 0- {}\n".format(
                key,
                self.get_coverage(key, Coverage.COVERAGE_ALL),
                self.get_coverage(key, Coverage.COVERAGE_MORE_9),
                self.get_coverage(key, Coverage.COVERAGE_MORE_0),
            )
        return sz_return

    def get_fault_message_9(self, element_name):
        return "Fail, locus '{}': the % of locus size covered by at least 10-fold is '{}%' (below 100%)".format(
            element_name, self.ratio_value_9
        )
    ...
    def get_fault_message_0(self, element_name):
        return "Fail, locus '{}': the % of locus size covered by at least 1-fold is '{}%' (below 100%)".format(
            element_name, self.ratio_value_0
        )
    ...
    def get_message_to_show_in_web_site(self, sample_name, element):
        ...
        if not self.limit_defined_by_user is None:
            return "Sample name: {}\nLocus: {}\n\nMean depth of coverage: {}\n% of size covered by at least 1-fold: {}%\n% of size covered by at least {}-fold: {}%".format(
                sample_name,
                element,
                self.get_coverage(element, self.COVERAGE_ALL),
    ...
    def get_icon(self, element, limit_to_mask_consensus):
        ...
        if not self.limit_defined_by_user is None:  ### not defined by user
            if self.is_100_more_defined_by_user(element):
                return os.path.join(
                    "/" + Constants.DIR_STATIC,
                    Constants.DIR_ICONS,
                    Constants.ICON_GREEN_16_16,
                )
        elif not self.limit_defined_to_project is None:
            if self.is_100_more_defined_to_project(element):
                return os.path.join(
                    "/" + Constants.DIR_STATIC,
                    Constants.DIR_ICONS,
                    Constants.ICON_GREEN_16_16,
                )
        elif self.is_100_more_9(element):
            return os.path.join(
                "/" + Constants.DIR_STATIC,
                Constants.DIR_ICONS,
                Constants.ICON_GREEN_16_16,
        ...
        if limit_to_mask_consensus > 0 and self.ratio_value_coverage_bigger_limit(
            element, limit_to_mask_consensus
        ):
            return os.path.join(
                "/" + Constants.DIR_STATIC,
                Constants.DIR_ICONS,
                Constants.ICON_YELLOW_16_16,
            )
        return os.path.join(
            "/" + Constants.DIR_STATIC, Constants.DIR_ICONS, Constants.ICON_RED_16_16
        )
...
class CountHits(object):
    ...
    def is_mixed_infection_ratio_test(self):
        ...
        """
        if (
            self.get_mixed_infection_ratio() > 0.5
            and self.get_mixed_infection_ratio() < 2.0
            and self.get_total_50_50_90() > 20
        ):
            return True
        return False
    ...
    def total_grather_than_mixed_infection(self):
        ...
        """
        if self.get_total_50_50_90() > CountHits.TOTAL_GRATHER_THAN_MIXED:
            return True
        return False
    ...
    def __eq__(self, other):
        return (
            other != None
            and other.hits_50_90 == self.hits_50_90
            and other.hits_more_90 == self.hits_more_90
            and other.hits_less_50 == self.hits_less_50
        )

    def __str__(self):
        return "hits_more_90: {}  hits_50_90: {}   hits_less_50: {}".format(
            self.hits_more_90, self.hits_50_90, self.hits_less_50
        )
...
class TasksToProcess(object):
    ...
    def add_taskd_id(self, task_id):
        if task_id not in self.vect_tasks_id:
            self.vect_tasks_id.append(task_id)
    ...
    def __eq__(self, other):
        if other == None or len(other.get_tasks_id()) != len(self.vect_tasks_id):
            return False
        for value_ in self.vect_tasks_id:
            if value_ not in other.vect_tasks_id:
                return False
        return True
...
class Gene(object):
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and self.end == other.end
            and self.strand == other.strand
            and self.vect_feature_locations == other.vect_feature_locations
        )
    ...
    def get_seq_feature(self):
        if self.has_feature_locations():
            ...
            try:  ## some files doesn't have this well formated
                ...
                pass
        return SeqFeature(
            FeatureLocation(self.start, self.end, strand=self.strand), type="CDS"
        )
...
class GeneticElement(object):
    ...
    def add_gene(self, element_name, length, gene):
        if element_name in self.dt_elements:
            if gene == None:
                return False
            if gene not in self.dt_elements[element_name]:
                self.dt_elements[element_name].append(gene)
            else:
                return False
        else:
            if gene == None:
                self.dt_elements[element_name] = []
            else:
    ...
    def get_genes(self, element_name):
        if element_name in self.dt_elements:
            return self.dt_elements[element_name]
        return None
    ...
    def has_genes(self, element_name):
        if element_name in self.dt_elements:
            return len(self.dt_elements[element_name]) > 0
        return None

    def get_size_element(self, element_name):
        if element_name in self.dt_elements_size:
            return self.dt_elements_size[element_name]
        return None

    def get_mask_consensus_element(self, element_name):
        if element_name in self.dt_elements_mask:
            return self.dt_elements_mask[element_name]
        return None
    ...
    def has_masking_data(self):
        ...
        for element in self.dt_elements_mask:
            if self.dt_elements_mask[element].has_data():
                return True
        return False
    ...
    def get_message_mask_to_show_in_web_site(self):
        ...
        for element in self.get_sorted_elements():
            if len(sz_return) > 0:
                ...
                sz_return += "#####################\n"
            sz_return += self.dt_elements_mask[element].get_message_to_show_in_web_site(
                element
            )
        return sz_return
    ...
    def get_message_to_show_in_csv_file(self):
        sz_return = ""
        for element in self.get_sorted_elements():
            if not element in self.dt_elements_mask:
                continue
            sz_out = self.dt_elements_mask[element].get_message_to_show_in_csv_file(
                element
            )
            if len(sz_out) > 0:
                if len(sz_return) > 0:
    ...
    def __eq__(self, other):
        if other == None or len(other.dt_elements) != len(self.dt_elements):
            return False
        for value_ in self.dt_elements:
            if (
                value_ not in other.dt_elements
                or other.dt_elements[value_] != self.dt_elements[value_]
            ):
                return False
        for value_ in self.dt_elements_size:
            if (
                value_ not in other.dt_elements_size
                or other.dt_elements_size[value_] != self.dt_elements_size[value_]
            ):
                return False
        for value_ in self.dt_elements_mask:
            if (
                value_ not in other.dt_elements_mask
                or other.dt_elements_mask[value_] != self.dt_elements_mask[value_]
            ):
                return False
        return True
    ...
    def __str__(self):
        ...
        for element_name in self.dt_elements:
            if len(self.dt_elements[element_name]) == 0:
                sz_out += "-- E.Name: {} ".format(element_name)
            else:
                sz_out += ";; Name: {}; Gene: {}; ".format(
                    element_name,
                    (
                        ""
                        if len(self.dt_elements[element_name]) == 0
                        else "-".join(
                            [str(gene) for gene in self.dt_elements[element_name]]
                        )
                    ),
                )
        return sz_out
...
class MixedInfectionMainVector(object):
    ...
    def __init__(self):
        ...
        Constructor
        """
        self.vect_start_compare = [
            [b for b in a] for a in ConstantsMixedInfection.vect_start_compare
        ]

    def add_vector(self, vector):
        if vector not in self.vect_start_compare:
            self.vect_start_compare.append(vector)
    ...
    def __str__(self):
        return "; ".join(
            ["-".join([str(b) for b in a]) for a in self.vect_start_compare]
        )
...
class ProcessResults(object):
    ...
    def get_error(self, index):
        if index < len(self.vect_results):
...
class MaskingConsensus(object):
    ...
    def set_mask_sites(self, mask_sites):
        ...
        for data_ in lst_data:
            if is_integer(data_) and not abs(int(data_)) in vect_data:
                vect_data.append(abs(int(data_)))
    ...
    def _clean_mask_regions(self, mask_regions):
        ...
        for data_ in lst_data:
            ...
            if (
                len(lst_positions) == 2
                and is_integer(lst_positions[0])
                and is_integer(lst_positions[1])
            ):
                pos_1 = abs(int(lst_positions[0]))
                pos_2 = abs(int(lst_positions[1]))
                if pos_1 > pos_2:
                    vect_data.append([pos_2, pos_1])
                elif pos_1 < pos_2:
                    vect_data.append([pos_1, pos_2])

        ## merge data
        if len(vect_data) > 0:
            vect_data = self._merge(vect_data)
            self.mask_regions = ",".join(
                ["{}-{}".format(data_[0], data_[1]) for data_ in vect_data]
            )
        else:
            self.mask_regions = ""
    ...
    def get_header(self, separator=","):
        """return header"""
        return "Mask sites{}Mask from beginning{}Mask from end{}Mask regions".format(
    ...
    def get_message_to_show_in_web_site(self, element_name):
        ...
        if not self.has_data():
            return "Element:{} -> No mask".format(element_name)
    ...
    def get_message_to_show_in_csv_file(self, element_name):
        ...
        if len(self.mask_regions) > 0:
            sz_out += "{}Mask regions:{}".format(
                " " if b_add else "(", self.mask_regions
            )
            b_add = True
        if len(self.mask_from_beginning) > 0:
            sz_out += "{}Mask from beginning:{}".format(
                " " if b_add else "(", self.mask_from_beginning
            )
            b_add = True
        if len(self.mask_from_ends) > 0:
            sz_out += "{}Mask from end:{}".format(
                " " if b_add else "(", self.mask_from_ends
            )
        return sz_out + ")"

    def has_data(self):
        return (
            len(self.mask_sites) > 0
            or len(self.mask_from_beginning) > 0
            or len(self.mask_from_ends) > 0
            or len(self.mask_regions) > 0
        )
    ...
    def __eq__(self, other):
        return (
            other != None
            and other.mask_regions == self.mask_regions
            and other.mask_from_ends == self.mask_from_ends
            and other.mask_from_beginning == self.mask_from_beginning
            and other.mask_sites == self.mask_sites
        )

    def __str__(self):
        return "mask_sites: {}  mask_from_beginning: {}   mask_from_ends: {}".format(
            self.mask_sites, self.mask_from_beginning, self.mask_from_ends
