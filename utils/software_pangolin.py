...
class SoftwarePangolin(object):
    ...
    def run_pangolin_update(self):
        ...
        try:
            ...
            try:
                ...
                self.logger_production.error("Fail to save: " + str(e))
                self.logger_debug.error("Fail to save: " + str(e))
    ...
    def _run_pangolin_update(self):
        ...
        if not settings.DEBUG:
            ...
            for line in vect_lines:
                ...
                if len(lst_data) > 1:
                    dt_result_version[lst_data[0]] = self._get_verion_tag(
                        lst_data[-1].replace("(", "").replace(")", "")
                    )
    ...
    def _get_verion_tag(self, description):
        ...
        for word in description.split():
            count_word = 0
            for char_ in word:
                if self.utils.is_integer(char_):
                    count_word += 1
                if count_word > 1:
                    return word
        return ""
    ...
    def run_pangolin(self, file_in_fasta, file_out_csv):
        ...
        cmd = "{} {} -o {} --analysis-mode {} -t 2".format(
            self.software_names.get_pangolin(),
            file_in_fasta,
            tem_dir,
        ...
        if not meta_sample is None:
            ...
            for soft_name in dt_pangolin_versions:
                ...
                if version_run_before is None:
                    return True
                if version_run_before != dt_pangolin_versions[soft_name]:
                    return True
            return False
        return True
            ...
            for soft_name in self.__get_order_pangolin_names(dt_pangolin_versions):
                ...
                if version_run_before is None:
                    ...
                    sz_out += "Do you want to run {} version ({})?".format(
                        soft_name, dt_pangolin_versions.get(soft_name)
                    )
                elif version_run_before != dt_pangolin_versions.get(soft_name):
                    if len(sz_out) > 0:
                        sz_out += "<br /><br />"
                    sz_out += (
                        "Do you want to update {} version from ({}) to ({})?".format(
                            soft_name,
                            version_run_before,
    ...
    def __get_order_pangolin_names(self, dt_pangolin_versions):
        ...
        for soft_name in dt_pangolin_versions:
            if (
                soft_name.startswith(SoftwareNames.SOFTWARE_Pangolin_name.lower())
                and not soft_name in vect_out
            ):
                vect_out.append(soft_name)
        for soft_name in dt_pangolin_versions:
            if not soft_name in vect_out:
                vect_out.append(soft_name)
        return vect_out
